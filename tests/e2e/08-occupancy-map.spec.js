...
test('Occupancy Map page loads and renders units', async ({ page }) => {
  ...
  const response = await page.goto('/admin/ALL/occupancy-map');
  ...
  await page.goto('/admin/ALL/occupancy-map');
  await page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
  await expect(page.locator('h1, h2').filter({ hasText: /Occupancy Map/i }).first()).toBeVisible();

  // Should render at least one unit tile (link to /admin/ALL/units/{id}/edit)
  const unitTile = page.locator('a[href*="/admin/ALL/units/"]').first();
...
test('Occupancy Map appears in admin sidebar nav', async ({ page }) => {
  ...
  await page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
  const navLink = page.locator('aside, nav').locator('a[href*="/admin/ALL/occupancy-map"]').first();
