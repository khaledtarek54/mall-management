...
function ppmOrder(array $attrs = [], int $items = 1): FacilityWorkOrder
{
...
it('raises a corrective job from a failed check', function () {
    ...
    expect($cm->work_order_type)->toBe(FacilityWorkOrder::TYPE_CM);
...
it('refuses to raise a corrective job from a check that has not failed', function () {
    $order = ppmOrder();
    $item = $order->items()->first();
    $this->wos->markItem($item, FacilityWorkOrderItem::RESULT_PASS);
...
it('refuses to raise a second corrective job for the same failed check', function () {
    ...
    expect(FacilityWorkOrder::corrective()->count())->toBe(1);
});
...
it('raises a follow-up linked to a closed order without reopening it', function () {
    ...
    expect($followUp->work_order_type)->toBe(FacilityWorkOrder::TYPE_CM);
