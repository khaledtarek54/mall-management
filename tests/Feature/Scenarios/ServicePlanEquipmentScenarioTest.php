...
function planFor(array $attrs = []): ServicePlan
{
...
it('throws rather than silently guessing when the stored unit is corrupt', function () {
    ...
    DB::table('service_plans')->where('id', $plan->id)->update(['frequency_unit' => 'decades']);
...
it('does not let one corrupt plan stop every other property getting work orders', function () {
    ...
    DB::table('service_plans')->where('id', $bad->id)->update(['frequency_unit' => 'decades']);

    $raised = $this->gen->run('2026-07-04');

    expect($raised)->toBe(1);
    expect($this->gen->failures)->toHaveKey($bad->id);
    expect(FacilityWorkOrder::where('service_plan_id', $good->id)->count())->toBe(1);
    expect(FacilityWorkOrder::where('service_plan_id', $bad->id)->count())->toBe(0);
});

/* ---- FR-PPM-01: Routine vs Fixed --------------------------------------- */

it('defaults a plan to routine maintenance', function () {
    expect(planFor()->plan_type)->toBe(ServicePlan::MAINTENANCE_TYPE_ROUTINE);
});
...
it('requires fixed maintenance to name its machine', function () {
    ...
    expect(fn () => planFor(['plan_type' => 'fixed', 'equipment_id' => null]))
...
it('accepts fixed maintenance against a machine', function () {
    $plan = planFor(['plan_type' => 'fixed', 'equipment_id' => $this->chiller->id]);
...
it('rejects an unknown maintenance type', function () {
    expect(fn () => planFor(['plan_type' => 'occasional']))
        ->toThrow(InvalidArgumentException::class, 'Unknown plan_type');
});
...
it('keeps generating after its machine is hard-deleted', function () {
    ...
    $plan = planFor(['plan_type' => 'fixed', 'equipment_id' => $this->chiller->id, 'checklist' => ['Check']]);
...
it('refuses to move a machine that has work-order history', function () {
    ...
    ServicePlan::query()->forceDelete(); // history outlives the plan
...
it('carries the plan\'s equipment onto every work order it raises', function () {
    planFor(['plan_type' => 'fixed', 'equipment_id' => $this->chiller->id, 'checklist' => ['Check pressure']]);
...
it('keeps saying which machine the job was against after the plan is gone', function () {
    // equipment_id lives on the order, not read through the plan: service_plan_id is
    ...
    expect($order->fresh()->service_plan_id)->toBeNull();
...
it('leaves equipment null on a property-wide plan', function () {
    ...
    expect(FacilityWorkOrder::first()->equipment_id)->toBeNull();
});
