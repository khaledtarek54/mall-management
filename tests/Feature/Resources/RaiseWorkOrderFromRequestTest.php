...
it('raises a linked work order from the action', function () {
    ...
    $wo = MaintenanceWorkOrder::where('tenant_request_id', $this->request->id)->sole();
...
it('hides the action from a role that can see requests but not create work orders', function () {
    ...
    // preventive_maintenance.create — triaging a ticket and raising facility work are different
