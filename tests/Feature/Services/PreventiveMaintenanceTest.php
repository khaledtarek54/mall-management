...
function makePlan(array $attrs = []): MaintenancePlan
{
...
it('raises a work order with the checklist for a due plan and advances next_due', function () {
    ...
    $order = MaintenanceWorkOrder::where('maintenance_plan_id', $plan->id)->first();
...
it('is idempotent — a second run does not double-generate', function () {
    ...
    expect(MaintenanceWorkOrder::where('maintenance_plan_id', $plan->id)->count())->toBe(1);
});
...
it('skips a plan that is not yet due', function () {
    ...
    expect(MaintenanceWorkOrder::count())->toBe(0);
});
...
it('runs the scheduled command', function () {
    makePlan();
    $this->artisan('maintenance:generate-preventive')->assertExitCode(0);
    expect(MaintenanceWorkOrder::count())->toBe(1);
});

it('mints distinct references for work orders in the same asset/month', function () {
    $asset = makeAsset();
    $a = MaintenanceWorkOrder::create(['asset_id' => $asset->id, 'title' => 'A', 'scheduled_for' => now()->toDateString()]);
    $b = MaintenanceWorkOrder::create(['asset_id' => $asset->id, 'title' => 'B', 'scheduled_for' => now()->toDateString()]);
