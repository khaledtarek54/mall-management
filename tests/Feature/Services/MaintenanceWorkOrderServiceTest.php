...
function makeOrder(array $attrs = [], int $items = 0): MaintenanceWorkOrder
{
...
it('completes once every item is marked, stamping who and when', function () {
    ...
    foreach ($order->items as $item) {
        $this->svc->markItem($item, MaintenanceWorkOrderItem::RESULT_PASS, $engineer->id);
    }
...
it('lets a visit that found a fault still close — a fail is marked, not blocking', function () {
    ...
    $order = makeOrder(items: 2);
    $this->svc->markItem($order->items()->first(), MaintenanceWorkOrderItem::RESULT_PASS);
    $this->svc->markItem($order->items()->orderByDesc('id')->first(), MaintenanceWorkOrderItem::RESULT_FAIL);
...
it('re-blocks completion when a marked item is set back to pending', function () {
    ...
    $this->svc->markItem($item, MaintenanceWorkOrderItem::RESULT_PENDING);
...
it('freezes the checklist of a terminal order', function () {
    ...
    expect(fn () => $this->svc->markItem($order->items()->first()->fresh(), MaintenanceWorkOrderItem::RESULT_PASS))
        ->toThrow(DomainException::class);
});
