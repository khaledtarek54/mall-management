...
| them both returned `INV-AW-202607-0082`. Reachable, too: invoices are created by five services,
...
it('holds the lock across the insert, not just the calculation', function () {
    ...
    $prefix = 'doc-number:INV-LCK-202607-';
    ...
    expect($rivalBlockedDuringInsert)->toBeTrue('the allocator must hold the prefix lock while inserting')
        ->and($invoice->number)->toStartWith('INV-LCK-202607-');
...
it('gives concurrent creations distinct numbers within one prefix', function () {
    // The property the whole mechanism exists to hold. Ten invoices in the same asset-month:
    // every number distinct, and the UNIQUE index never had to refuse anything.
    $asset = makeAsset();
    $lease = makeLease(makeUnit($asset), makeTenant());
    ...
    expect($numbers->unique())->toHaveCount(10)
        ->and($numbers->every(fn (string $n) => str_starts_with($n, 'INV-'.$asset->code.'-202607-')))->toBeTrue();
});
...
it('scopes the lock to the prefix, so one property never blocks another', function () {
    ...
    $lockA = Cache::lock('doc-number:INV-AAA-202607-', 5);
    ...
    expect($invoiceB->number)->toStartWith('INV-BBB-202607-');
