...
- **Model** (`app/Models/`): `$fillable`, `$casts`, and `$attributes` defaults for NOT-NULL booleans/numerics. Relationships. If financial, route money through `Invoice::recomputeTotals()` — never set `paid_amount`/`balance` directly. Use the `LogsActivity` trait if the module should appear in the activity log — then do **all four** steps in [modules/18 → Extension points](../../../docs/modules/18-rbac-scoping.md), not just the subject key: `admin.activity.subjects.{log_name}`, a label for every `logOnly()` column (reuse the `admin.fields.*` key the form uses), a `VALUE_VOCABULARY` entry for each enum-ish column, and a `FOREIGN_KEYS` entry for each `*_id` so the diff names the record instead of printing an id. All of it in **en AND ar**. `ActivityLogVocabularyConformanceTest` fails the build otherwise.
