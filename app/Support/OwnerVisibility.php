...
class OwnerVisibility
{
    ...
    public const OPERATOR_INTERNAL = [
        ...
        'tenant_request_subcategories' => 'The vocabulary Eltizam gives its tenants for reporting faults, and which trade each routes to. An owner sees the work that was done and what it cost; how the operator triages an incoming complaint is its own service design.',
        'retail_categories' => "The merchandising mix is arguably the one catalogue an owner has a stake in — it is the vocabulary his tenant-mix report is drawn in. Internal FOR NOW because the register is Eltizam's leasing vocabulary across the whole portfolio, not a fact about his property; reclassify if an owner ever needs to reconcile a mix report against the list behind it.",
        'fixed_asset_categories' => "How Eltizam classifies the assets it owns and depreciates — the number series, the proposed lives and the memo values. An owner sees the depreciation charged against his property on the statement; the operator's own asset classes are how it keeps its register.",
        'violation_categories' => 'The house rules an operator enforces on retailers, and the tariff behind them. An owner sees the OUTCOME — a fine billed, an income line — not the rule book Eltizam applies across the malls it runs; publishing it would invite a per-property negotiation of a portfolio-wide policy.',
        'vendor_document_types' => "What Eltizam requires of its own suppliers before letting them onto a floor. Internal: an owner has no supplier of his own here, and `blocks_dispatch` is the operator's liability decision, not a fact about his property.",
        'expense_categories' => "How Eltizam classifies its own costs, and which of its own P&L accounts each kind books to. An owner sees the operating costs charged against his property on the statement; the operator's internal chart of overhead is how it runs its business.",
        'payment_methods' => "How Eltizam takes and moves money — its rails, its settlement timing and which of its own accounts each lands in. An owner sees the rent that was collected and when; the operator's banking arrangements are how it runs its business, not a fact about the property.",
