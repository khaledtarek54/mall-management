...
class MaintenancePlan extends Model
{
    ...
    protected static function booted(): void
    {
        static::saving(function (self $plan) {
            ...
            if (! in_array($plan->maintenance_type, self::MAINTENANCE_TYPES, true)) {
                throw new InvalidArgumentException(
                    "Unknown maintenance_type '{$plan->maintenance_type}'; expected one of: ".implode(', ', self::MAINTENANCE_TYPES).'.'
                );
            }
            ...
            if (! $plan->isDirty(['asset_id', 'equipment_id', 'maintenance_type'])) {
                return;
            }

            // FR-PPM-01: Fixed maintenance is "per asset" — it must name its machine.
            if ($plan->maintenance_type === self::MAINTENANCE_TYPE_FIXED && $plan->equipment_id === null) {
