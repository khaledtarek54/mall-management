...
class JobBrief
{
    ...
    public static function of(FacilityWorkOrder $record): array
    {
        ...
        return [
            ...
            Section::make(__('vendor.jobs.brief.quotes'))
                ->visible(fn (): bool => $record->proposals()->exists())
                ->schema([
                    TextEntry::make('quotes')
                        ->hiddenLabel()
                        ->columnSpanFull()
                        ->state(fn (): string => $facts['quotes']),
                ]),
        ];
    }
    ...
    private static function thread(FacilityWorkOrder $record): string
    {
    ...
    /**
     * Who said it, without leaking anything the contractor should not have.
     *
     * The author is a MORPH — an operator or one of this company's own contacts — so a name has to
     * be asked of the row rather than assumed. An operator whose record has gone (a departed
     * colleague) reads as the operator generically: the alternative is a blank byline on a message
     * that plainly came from the mall.
     */
    private static function authorName(FacilityWorkOrderComment $comment): string
    {
        ...
        if ($author === null) {
            return __('vendor.jobs.brief.from_operator');
        }

        return (string) ($author->name ?? __('vendor.jobs.brief.from_operator'));
    }

    /** Every quote this contractor has sent, and what the operator decided. */
    private static function quotes(FacilityWorkOrder $record): string
    {
