    ...
    public static function fields(?int $assetId): array
    {
        return [
            ...
            Select::make('execution_type')
                ...
                ->default(MaintenanceWorkOrder::EXECUTION_INTERNAL)
                ->required()
                ->live()
                ->native(false),
            ...
            Select::make('assigned_to_user_id')
                ->label(__('admin.preventive_maintenance.cm.assignee'))
                ->helperText(__('admin.preventive_maintenance.cm.assignee_hint'))
                // Staff who can actually reach this property — never leak another mall's roster.
                ->options(fn () => self::technicianOptions($assetId))
                ->visible(fn (Get $get) => $get('execution_type') === MaintenanceWorkOrder::EXECUTION_INTERNAL)
                ->searchable()
                ->native(false),
