 ...
 */
class RaiseCorrectiveWorkOrderService
{
    /**
     ...
     */
    /** The stronger of two priorities, ranked by FacilityWorkOrder::PRIORITIES. */
    private static function higherPriority(string $a, string $b): string
    {
        $rank = fn (string $p): int => (int) array_search($p, FacilityWorkOrder::PRIORITIES, true);
     ...
     */
    public function fromFailedCheck(FacilityWorkOrderItem $item, array $data): FacilityWorkOrder
    {
        return DB::transaction(function () use ($item, $data) {
            ...
            /** @var FacilityWorkOrder $origin */
            $origin = FacilityWorkOrder::whereKey($item->facility_work_order_id)->lockForUpdate()->firstOrFail();
            $item->refresh();

            if (! $item->hasFailed()) {
                throw new DomainException(__('admin.facility.errors.cm_needs_failed_item'));
            }
            ...
            if (FacilityWorkOrder::where('source_item_id', $item->getKey())->exists()) {
                throw new DomainException(__('admin.facility.errors.cm_already_raised'));
            }
     ...
     *               vendor_id?:int|null, assigned_to_user_id?:int|null, scheduled_for?:string|null}  $data
     */
    public function asFollowUp(FacilityWorkOrder $original, array $data): FacilityWorkOrder
    {
        return DB::transaction(function () use ($original, $data) {
            /** @var FacilityWorkOrder $locked */
            $locked = FacilityWorkOrder::whereKey($original->getKey())->lockForUpdate()->firstOrFail();
     ...
     */
    public function fromTenantRequest(TenantRequest $request, array $data): FacilityWorkOrder
    {
        return DB::transaction(function () use ($request, $data) {
            ...
            if ($locked->unit === null) {
                throw new DomainException(__('admin.facility.errors.wo_needs_unit'));
            }
            ...
            return FacilityWorkOrder::create([
                ...
                'vendor_id' => $executionType === FacilityWorkOrder::EXECUTION_EXTERNAL ? ($data['vendor_id'] ?? null) : null,
                'assigned_to_user_id' => $executionType === FacilityWorkOrder::EXECUTION_INTERNAL ? ($data['assigned_to_user_id'] ?? null) : null,
            ]);
        });
    }
     ...
     */
    private function create(FacilityWorkOrder $origin, array $data, array $link): FacilityWorkOrder
    {
        $executionType = $data['execution_type'] ?? null;
        ...
        return FacilityWorkOrder::create(array_merge([
            ...
            'vendor_id' => $executionType === FacilityWorkOrder::EXECUTION_EXTERNAL
                ? ($data['vendor_id'] ?? null)
                : null,
            'assigned_to_user_id' => $executionType === FacilityWorkOrder::EXECUTION_INTERNAL
