@php
    ...
    $active = 'background:#0F766E;color:#F5F0E8;box-shadow:0 1px 2px rgba(0,0,0,0.12);';
    $inactive = 'background:transparent;color:#94A3B8;';
@endphp
<div style="display:inline-flex;align-items:center;gap:2px;padding:3px;margin-inline-start:0.75rem;border-radius:8px;border:1px solid rgba(15,118,110,0.4);background:rgba(15,20,25,0.55);"
     role="group" aria-label="Language">
    <a href="{{ route('locale.switch', 'en') }}"
       style="{{ $base }}{{ $isAr('en') ? $active : $inactive }}"
       aria-pressed="{{ $isAr('en') ? 'true' : 'false' }}"
       aria-label="English"
       onmouseover="if(!this.dataset.active){this.style.color='#14B8A6'}"
       onmouseout="if(!this.dataset.active){this.style.color='#94A3B8'}"
