...
| ~~S-8~~ ✅ | **FIXED 2026-08-21 (EG-16).** The logo reaches all twelve document templates through `IssuingEntity::forView()` — the one seam nine PDF services already call — and is rendered by ONE partial, so a thirteenth template cannot be added without it (gated). A local FILE PATH, never `logoUrl()`: mpdf renders server-side, and a URL makes every document depend on the box fetching its own public address, failing as a missing image nobody sees an error for. ~~No mall logo on any PDF~~ | `app/Support/IssuingEntity.php`; `resources/views/partials/issuer-logo.blade.php` | 🟠 |
