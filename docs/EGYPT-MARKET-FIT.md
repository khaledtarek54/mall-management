...
| **EG-30** | 🟡 **HALF DONE 2026-08-22 — M-6 shipped, M-2 open.** `leases.escalation_interval_months`, nullable, **null means twelve**, read through `Lease::escalationIntervalMonths()` — so a biennial clause, an 18-month step or a six-monthly fit-out review is a number on the lease instead of something an operator remembers, and every existing lease steps annually exactly as before. Two things the build got wrong first and the tests caught: Carbon's `addMonths()` OVERFLOWS a month-end date (31 Aug + 18 months → 2 March, not the last day of February), so the roll is `addMonthsNoOverflow()`; and a 0 written by an importer would roll the date nowhere and make the sweep reconsider that lease every day for ever, so the accessor floors at one month. **M-2 — billing in ARREARS — is not started**: `MonthlyBillingService` bills in advance with the cycle anchored to the lease, and a service charge or utility recharge billed in arrears still has no home. It is the larger half and touches periods, proration and the fit-out grace | M-2, M-6 | 🧑‍💻 | M |
