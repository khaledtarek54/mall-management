...
1. **Egyptian law moved under a rule the system treats as fixed.** Law 157/2025 pulled property rental
   ...
   executive regulations*. **Taxing rent is a CONFIGURATION act — the operator's own ruling,
   2026-08-23, and on this half they are right**: point charge code `base_rent` at a dated tax code on
   `/admin/charge-codes` and every rent line originated from that day bills it, on every lease already
   on the books (EG-01 stopped freezing taxability onto the charge row). No deploy, and no answer
   anyone has to hand us first. What is still portfolio-wide is the DIFFERENTIATION — *"this lease is
   taxed, that one is not"* — whose only channel today is a per-charge `vat_rate` override reachable
   through the Charges importer, and which moves the RATE rather than the tax CODE
   ([§4.1](#41-tax), EG-02).
...
| 2 | **Tax *treatment*** (who is taxed) | 🟠 SEMI | *Whether* a supply is taxed is a row, resolved at billing so a ruling reaches every existing lease — no deploy, no gate in the way (EG-01; T-4 measured false 2026-08-23). *Which leases* is the gap: the per-charge override is importer-only and moves the rate, not the tax code (EG-02) |
...
| 9 | **Master-data lists** | ✅ DONE | Trades, failure codes, charge codes, tax codes, SLA policies, rent indices, and the seven operator catalogues sharing `IsCodeCatalogue` — payment rails (EG-11), expense categories (EG-13), tenant-request subcategories (EG-14), the merchandising mix, house rules and supplier document types (EG-23), and the fixed-asset classes (2026-09-12) — are all rows, as are departments. The request TYPE stays an enum on purpose, because it carries behaviour |
...
that half is fixed and the missing dimension (T-1, EG-02) is not. **Re-measured 2026-08-23, after the
operator ruled that taxing rent is theirs to configure:** they are right, and T-4 — the row claiming a
conformance gate stood in the way — was false.

| # | Finding | Where | Rating |
|---|---|---|---|
| T-1 | **Taxability has no lease or unit dimension — though "there is no third input" was too strong.** `Vat::rateForType($type, $on)` resolves *charge code → tax code → dated rate*, one answer for the portfolio; `charges.vat_rate` overrides it per charge row, effective-dated, and `Charge::resolvedVatRate($on)` is what billing actually reads. Two things stop that being the dimension: for RENT the only channel to it is the Charges importer — the lease's charge-schedule screen refuses `base_rent` as a derived type — and it moves the RATE while the line keeps the charge code's `tax_code`, so a 1% schedule tax entered that way posts under the VAT role and files on the VAT return | `app/Support/Vat.php:125`; `app/Models/Charge.php:361`; `app/Filament/Imports/ChargeImporter.php:118` | 🟠 |
...
| ~~T-4~~ ❌ | **MEASURED FALSE 2026-08-23.** ~~Making base rent taxable turns a conformance gate red.~~ `ChargeCodeVatTreatmentConformanceTest` seeds `ChargeCodeSeeder` in its own `beforeEach` and compares THAT catalogue against the floor — it never reads the operator's database. So an accountant pointing `base_rent` at `VAT_14` on a live install turns nothing red, and no runtime guard refuses it: the floor answers only for a charge code with **no row**, and this one has a row. The PHP edit buys one narrower thing — changing what a **fresh install ships**, where the seeded catalogue and the unseeded floor must keep agreeing. *"Change the tax treatment of rent"* **is** a no-deploy operation | `tests/Feature/Scenarios/ChargeCodeVatTreatmentConformanceTest.php:27-42`; `app/Support/Vat.php:95,125` | ✅ |
...
| T-7 | **Tenant-side WHT unmodelled** — a tenant who withholds from rent reconciles as an underpayment forever | `docs/STATUS.md` A2.1 | 🟠 |
...
> **The concrete Egyptian scenario, re-measured 2026-08-23 — and (b) was wrong.** Suppose the
> executive regulations land and the accountant rules: *administrative units in the mall carry 1%
> schedule tax; retail shops stay exempt.* Taxing rent **at all** needs nothing from a developer:
> point `base_rent` at the schedule-tax code, dated, and it reaches every lease on the books.
> ~~(b) a PHP edit to `Vat::EXEMPT_TYPES` and the gate~~ is needed only to change what a **fresh
> install ships** — the gate seeds its own catalogue and never reads the operator's (T-4).
> ~~and (c) **it still would not reach any existing lease**, because every rent charge row already
> carries `vat_applicable = false` frozen at creation~~ **was fixed 2026-08-22 (EG-01)**. What is
> genuinely left is (a) and the DIFFERENTIATION behind it: two units in one mall taxed differently,
> and taxed under different CODES rather than merely at different rates. That is a third input to the
> resolver — EG-02 — and it is plain code, waiting on nobody's ruling.
...
| X-11 | **Notification routing is ~15 PHP literals.** No screen, no table, no settings group; `super_admin` is hard-unioned into every operator fan-out; there is no opt-out and no per-tenant channel choice | `app/Services/TenantRequestService.php:200`; `app/Services/AssetStaffRecipients.php:34` | 🟠 |
...
| # | Finding | Where | Rating |
|---|---|---|---|
| S-1 | **There is no legal entity.** No model, no table, no column. Everything is one company. `docs/STATUS.md:159` already states the consequence for the Jawad/Eltizam revenue split | absence proven; `app/Support/IssuingEntity.php:27-30` | 🔴 |
...
| **EG-02** | **Give tax treatment a lease/unit dimension.** An effective-dated tax treatment on the lease (or on the unit's type), resolved through `Vat::rateForType()` as a third input, so *"admin units taxed, retail exempt, from date D"* is expressible — and expressible as a tax **CODE**, not merely a rate, so a schedule tax stops filing itself as VAT. **The 🔑 came off 2026-08-23**: the operator ruled that the accountant adds tax on rent from the screens, and for the portfolio-wide half that is exactly right — charge code → dated tax code, no deploy, reaching every existing lease (and T-4, the row claiming a gate stood in the way, was measured false the same day). What remains is code and waits on no answer: the DIFFERENTIATION, whose only channel today is a per-charge `vat_rate` override reachable through the Charges importer | T-1, §3.1 | 🧑‍💻 | L |
...
| **EG-29** | ✅ **DONE 2026-08-23.** `App\Support\ProrationMethod` — Yardi's four (`actual` · `thirty_day` · `year_365` · `whole_month`) on the usual **lease → property → portfolio** tiers, `actual` everywhere by default so nothing an install bills moved. **The METHOD is per LEASE; whether a row prorates at all is per CHARGE** — `charges.prorate`, added 2026-08-23 when the staging verification against Yardi found the original "deliberate deviation" here was simply wrong (Yardi's charge row carries a prorate flag, and a flat parking fee billed 2,580.65 of 5,000 on a mid-month move-in). A non-prorating row resolves to the existing `WHOLE_MONTH` rule, never a new branch, so the termination credit reads the same definition back and does not claw a flat charge back either. **A full month is exactly one month under every method**, or 30/360 bills 31/30 of a month every August. Threaded through all five `monthsCovered()` call sites from one resolution, and `CreditUnearnedBillingService` reads the AGREEMENT's method so a termination credit cannot disagree with the invoice it credits — which is why `prorationMethod()` sits on `BillableAgreement` and a unit ownership answers it too. **CAM deliberately excluded**: it weights area × days ÷ period days, and numerator and denominator must share one day basis | M-1 | 🧑‍💻 | M |
...
| **EG-32** | ✅ **DONE 2026-08-23 — four slices.** **S-5's user-defined columns are real and durable**: every table is toggleable AND reorderable panel-wide, a saved list view stores the toggles and the ORDER, and the 23 catalogued reports remember their layout too (`SavedColumnLayout`, one implementation for both). **D-7 is complete end to end** — define, fill, read, filter, sort, export, import and search. **What this is NOT is a drag-and-drop report DESIGNER**, and that is stated rather than implied: which columns a given report offers as optional is a per-report editorial call (RentRoll offers 3; AR ageing, the GL, the expiration schedule and the income statement offer none), and a generic query builder over financial statements, an occupancy map and a workflow diagram is not a sensible thing to build. **SLICES 1–3. Slice 3 made the answers USABLE** — list column, filter and sort per field on all five lists, and export columns on the three resources that have an exporter. A value you can type and never group by is the notes box with extra steps. **SLICE 2 — custom fields (D-7) are built**, the larger half. **SLICE 1 DONE 2026-08-23 — a saved view remembers its columns.** The cheapest real part of S-5, and the part the finding got wrong: columns were already user-selectable, just not durable or shareable. **Still open and still XL:** a report BUILDER for the 23 catalogued report pages, user-defined groupings, column reordering (needs a blank-label sweep — two table columns use `->label('')` and `reorderableColumns()` throws on them), **SLICE 5 (2026-08-23) — the DEFAULT view**, which UX-11 asked for in writing (*"save a table's filter+column state **and set one as their default**"*) and which the benchmark row had been ticked without: `table_views.is_default`, a personal default beating a team one, applied as a REDIRECT to the view's own URL so a view stays a link. **Nothing open beyond one editorial choice**: widening which report and list columns are offered as optional. The vendor and property exporters — the last concrete gap — shipped the same day | S-5, D-7 | 🧑‍💻 | XL |
| **EG-33** | ✅ **DONE 2026-08-23 — both kinds of standing cost.** **The supplier half shipped the same day, after the pre-staging verification found it missing**: a schedule naming a VENDOR raises a DRAFT `VendorBill` instead of an expense — the fixed cleaning, security and lift-maintenance retainers a mall has most of, which `vendor_contracts` generated nothing for. `expenses` carries no `vendor_id` at all, so naming a supplier IS the statement that the cost is a payable, and there is no second `type` column to disagree with it. DRAFT because `vendor_bills.reference` is the supplier's own invoice number and cannot be invented, and posting Dr Expense / Cr AP for an invoice nobody sent invents a creditor's claim — Voyager stages its recurring payable batch for the same reason. Also fixed: `tax_code` was offered on the form and read by NOTHING, so a cost under VAT_14 booked no recoverable input VAT. `recurring_expenses` + `expenses:generate-recurring` (daily) + `/admin/recurring-expenses` — Yardi's **Recurring Payables**, which this system had no counterpart to at all: recurrence existed only on the revenue side, so every cost arriving on a calendar was somebody's reminder. Monthly · quarterly · **semiannually** (Egyptian real-estate tax is two instalments) · annually. **The schedule is NOT a GL source** — it mints an `Expense`, which posts through the journalizer that already exists; registering the schedule would post every levy twice and balance both times. **Idempotent three ways**: `lockForUpdate`, the due date re-derived INSIDE the transaction after the lock, and `expenses.(recurring_expense_id, expense_date)` UNIQUE underneath. **It catches up ONE period per run**, so a schedule switched back on after six months does not post six back-dated entries into possibly-closed periods. **What is deliberately NOT modelled: the tax itself** — the rate, the rental-value basis, the 32% non-residential deduction and the assessment are facts the operator holds, and a computed guess would put a confident wrong number on a statutory filing. That half stays with the accountant | T-8, §3.6 | 🧑‍💻 + 🔑 | M |
...
[STATUS.md](STATUS.md), which is the single list** (2026-08-23) — recorded below with
...
1. ~~**Which units in the mall carry the 1% schedule tax on administrative-unit rental, and from what
   ...
   for us.** The accountant adds the tax on rent from the screens: `base_rent` → a dated tax code is a
   row on `/admin/charge-codes`, it reaches every lease already on the books, and nothing in the code
   refuses it (T-4, which said a conformance gate did, was measured false the same day). It stops being
...
| Doc | Says | Verdict |
|---|---|---|
| ~~`docs/STATUS.md` A1.1~~ | ~~The VAT rate is `TaxSettings::vat_standard_rate`~~ | ❌ **STALE FINDING.** A1.1 already reads *"the rate is now MASTER DATA … `TaxSettings::vat_standard_rate` was removed 2026-08-12"*. Corrected before this list was worked |
| ~~`docs/STATUS.md` A3.2~~ | ~~"no straight-line spread"~~ | ❌ **STALE FINDING.** A3.2 already reads *"Straight-line rent IS built … ships off behind `BillingSettings::straight_line_rent_enabled`"* |
...
Of the eleven rows: **two were already fixed** elsewhere (both `STATUS.md` entries), **one
...
taxed, retail exempt, from date D"* — is the separate work. *(And since 2026-08-23 it waits on nobody:
the operator ruled that taxing rent is theirs to configure, which is true of the portfolio-wide half,
leaving EG-02 as plain code.)*
...
### 2026-08-23 — milestone 30: EG-33, the costs that arrive on a calendar

**There was no recurring-expense concept anywhere in this system.** Recurrence existed only on the
revenue side — `charges` bill a lease every cycle — so every cost that arrives on a calendar rather
than on an invoice was somebody's reminder: real-estate tax instalments, municipal levies, the
annual civil-defence licence, a fixed cleaning retainer. Yardi calls these **Recurring Payables** and
has shipped them for twenty years, because a property company's outgoings are mostly known in
advance.

`recurring_expenses` + `GenerateRecurringExpensesService` + `expenses:generate-recurring` (daily) +
a screen at `/admin/recurring-expenses`. Monthly, quarterly, **semiannually** — that one earns its
place, Egyptian real-estate tax being payable in two instalments — and annually.

**The schedule is NOT a money document.** It mints an `Expense`, and THAT posts through the
journalizer that already exists — verified end to end on the real database, `JE-2026-0562` against
`EXP-AW-2026-0008`. Registering the schedule in `LedgerPoster::JOURNALIZERS` would post every levy
twice and balance both times, which is the same reasoning that keeps a facility work order a cost
object rather than a GL source.

**Booking a statutory cost twice is real money, so it is belt and braces**: the schedule row is
taken with `lockForUpdate()`, the due date is re-derived INSIDE the transaction *after* the lock —
a value read before the wait is answered from a snapshot taken before the other writer committed —
and `expenses.(recurring_expense_id, expense_date)` is UNIQUE underneath, so even a bug fails loudly
rather than double-booking.

**It catches up ONE period per run.** A schedule switched off for six months and back on must not
mint six back-dated expenses in a night: six journal entries into periods that may be closed, on a
cost nobody re-reads. It books the oldest outstanding period and asks again tomorrow.

**The day is clamped to the month**, so a schedule set to the 31st books on 28 February rather than
skipping the seven shorter months — the trap `BillingDay` already records.

**What is deliberately NOT modelled is the tax itself.** Egyptian real-estate tax has a rate, a
rental-value basis, a 32% non-residential maintenance deduction and an assessment issued per
property. None of it is invented here: the assessed figure is a fact the operator holds, and a
system that computed it from guessed rates would put a confident wrong number on a statutory filing.
**This is the schedule; the amount is theirs.** That is the surviving 🔑 in this row.

10 cases, three mutations checked: dropping the idempotency stamp fails four, ignoring the end date
fails one, un-clamping the day fails one. One asserts the SCHEDULER runs the command, because
`BillUnitOwnershipsService` was fully built, fully tested, and billed nobody for the whole of module
37's life.
...
**The METHOD is per LEASE; whether a row prorates AT ALL is per CHARGE (corrected 2026-08-23).**
This section originally read *"per lease, not per charge code — no clause these malls have signed
prorates rent one way and the service charge another"*, and called that a deliberate deviation. The
staging verification against Yardi found it wrong. Yardi's lease charge row carries a **prorate
flag** — *"charge code · amount · from date · to date · frequency · basis · prorate flag"*
([01-yardi-lease-administration.md](benchmarks/yardi/01-yardi-lease-administration.md) §3.2) — and
the question it answers is not *which* method but *whether*. A flat signage licence, a fixed parking
fee or a fixed management fee is payable in full for any month the lease runs into; taking it from
the 15th does not make it half a licence. Measured on a 16 August commencement, a 5,000 parking fee
billed **2,580.65** and the termination credit clawed back another 2,500 it had earned.

`charges.prorate` closes it — nullable, null and `true` meaning *prorate by the lease's method*, and
a non-prorating row resolving to the EXISTING `WHOLE_MONTH` rule rather than a new branch, so the
termination credit reads the same definition back. The method stays a lease term: that half was
right. See [modules/05](modules/05-billing-invoices.md).
