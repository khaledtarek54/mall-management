...
- **A tab is a narrowed copy of its register — same colours, same acts, one definition each.** `App\Support\BadgeColors` is the one map from a `ValueSets` value to its badge colour (measured 2026-09-11: 87 inline maps, 7 vocabularies disagreeing with themselves across the register, its tabs and the portal); `App\Filament\Actions\OpenRecordAction` is the one *Open* on a relation manager's row (twelve hand-written copies in four shapes, one a viewer's link into a 403), resolving edit-then-view per record in the row's own mall, and `RowClickTarget` makes the row click follow it on a tab; an act offered on two surfaces is a `*Actions` registry composed as one spread (`App\Filament\Portal\Actions\InvoiceActions`, `App\Filament\Actions\RentableItemHoldingActions`). Gates: `BadgeColorsConformanceTest`, `ATabOpensARecordThroughOneFactoryConformanceTest`, `AnActIsDeclaredOnceConformanceTest`. See [docs/benchmarks/yardi/08 → rule 7](benchmarks/yardi/08-yardi-ui-ux.md#one-colour-one-open).
