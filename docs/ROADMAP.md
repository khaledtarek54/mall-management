...
| **UX-25** | **Deferred loading** — **MEASURED, NOT TAKEN.** `deferLoading()` is genuinely absent (0 occurrences, and a real `ListInvoices` table answers `isLoadingDeferred = false`). Full page renders on demo data: invoices 86 queries, leases 272, work orders 111, with work orders the slowest screen in the panel. Deferring would paint the chrome first and fetch after — at the price of a second round trip on every list view, and of breaking every test that reads rows through the shared `tableRows()` helper, since a deferred table returns nothing until `loadTable` fires. Take it per-table where a specific list proves slow in a browser, not panel-wide. The real first-paint win was UX-24's, and it needed no trade. |
| **UX-26** | **The technician's screens on a phone** — **STILL OPEN, and the row overstated the neglect.** `resources/css/filament/theme.css` already carries a deliberate mobile block: form controls are pinned to 16px below 767px because iOS Safari zooms the viewport on focus and the operator has to pinch back out after every field. So somebody has thought about phones. What has never been done is driving the actual WORKFLOW — Work orders → labour → parts → completion evidence — on a handset, and the two widest tables in the panel (work orders and tenant requests, 19 columns each) are exactly the ones a technician lives in. Since technicians use the admin panel by decision (O3, DECLINED), that role's phone experience is a requirement rather than a nicety. Needs a browser at a phone viewport; the manifest-driven Playwright suite already knows every screen. |
