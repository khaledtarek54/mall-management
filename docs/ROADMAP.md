...
| **CFG-05** | ✅ | 🧑‍💻 | **SHIPPED 2026-08-12.** `/admin/configuration-health` — seven checks that read the LIVE database (payroll added 2026-08-20, EG-04), each saying **what the gap breaks** rather than which field is empty ("tenants cannot reclaim the VAT you charged them", not "seller_tax_registration_number is empty"). `atriom:health` answers *is it alive*; this answers *is it set up*, and the two fail differently — a perfectly healthy install bills through a floor rate because nobody classified the charge codes. Severity is about money and law, not tidiness. Every check is tested from both sides: a checklist that reports all-clear because its detection is broken is a green light nobody earned. *(Original row below.)* **Configuration health page** — what is unset and what it breaks; the in-app form of [STATUS.md](STATUS.md), and the analogue of Yardi's setup checklist. `atriom:health` already computes most of it. |
