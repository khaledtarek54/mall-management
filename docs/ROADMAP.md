...
| 5 | **Inventory & warehousing** — low-stock alerts, bins, transfers | 🟡 `part_source` internal/external + the parts draw shipped (`01ea84a`). **⚠️ Two of this row's three claims were STALE — re-verified against the code 2026-08-18, do not rebuild either.** The portfolio-wide on-hand sum is **fixed**: `InventoryItemResource::getEloquentQuery()` scopes `withSum` through `warehouse_id → asset_id` and carries a comment describing the exact leak (a manager on mall A seeing mall B's 100 and a green reorder colour). Transfers are **built, not dead**: `StockMovementService::transfer()` writes both legs in one transaction, out-first so an impossible transfer is refused before anything is written, and `InventoryMovementJournalizer` handles the pair. **Genuinely remaining:** low-stock alerts (FR-INV-03) and bins. |
