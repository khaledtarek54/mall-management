...
> has to make are **[STATUS.md](STATUS.md)**, which
> `/admin/configuration-health` now checks eight rows of automatically.
>
> **Nor is it the configurability list.** *What the operator can change without a developer*, measured
...
> over-accruing the employer share. **EG-01 — taxability frozen onto the charge row — shipped 2026-08-22**: `vat_applicable`
...
| **Flip ETA out of mock mode** | `config/eta.php` → `ETA_MOCK` (default true) submits to a fake endpoint — no legally-binding invoices. One env flip, gated on the two rows above. *(`EtaSettings` was deleted 2026-08-22 with the module freeze — it was inert; the flag only ever lived in config.)* | 🧑‍💻 | S |
...
| **`integrations:check` preflight** | Run `php artisan integrations:check` after the live `.env` swap to validate Paymob + mail creds before the first real charge. Command is built and exits non-zero on failure. **ETA is skipped by default since the 2026-08-22 freeze** — reporting on an integration that cannot run is the same false signal the settings tab was; pass `--eta` to check the dormant credentials on purpose. | ⚙️ | S |
...
| **Paymob credential vaulting + HMAC rotation** | 🟡 **The ROTATION half shipped 2026-08-24; the VAULTING half is still ops.** The row said a dual-secret verifier was *"worth building only alongside a rotation procedure that someone will actually run"* — so both were built together. **The problem it solves:** Paymob signs each callback with whatever secret their dashboard holds at that instant, so the moment you change it there, every callback in flight and every retry they have not had a 200 for is still signed with the OLD secret — refused by a single-secret verifier, and **a refused callback is a payment the tenant made that the books never see**. `PAYMOB_HMAC_SECRET_PREVIOUS` + `PAYMOB_HMAC_PREVIOUS_UNTIL` accept both for a bounded window; the bound is what makes it a rotation rather than a permanent second key, and an unparseable date counts as CLOSED (a typo must narrow what is accepted, never widen it). `atriom:health` gains `paymob_hmac_rotation`: silent normally, **OK while the window is open** (a check that fires during the procedure it supports trains people to ignore it), and FAILS in production once the window closed with the secret still in `.env`. Procedure: [PAYMOB-SETUP.md §7](integrations/PAYMOB-SETUP.md). Three mutations — unbounded previous secret, unparseable-means-open, fail-open with no secret — each turn `PaymobHmacRotatesWithoutDroppingACallbackTest` red. **Still outstanding and NOT code:** the credentials live in plaintext `.env` with no vault. That is a secrets-manager decision, or at minimum file permissions and a named owner. *(Everything the 2026-08-11 verification found is unchanged and still true: timing-safe compare, fails closed, replay is a no-op, the terminal-status check is inside the transaction under a lock.)* |
...
| ~~**3**~~ | ~~**Automated rent escalation**~~ ✅ **SHIPPED (2026-07-19)** | `RentEscalationService` + `leases:apply-escalations` (scheduled daily): a lock-safe, idempotent sweep over active `fixed_percent` leases with `next_escalation_date ≤ today` — applies the increase through `LeaseRentChangeService` (base-rent Charge + marketing levy synced) and rolls the date forward a year. ~~**CPI is skipped** (no index feed — inventing a number would be inventing data).~~ ✅ **CORRECTED — CPI shipped 2026-08-19 (`e8a04469`), re-verified against the code 2026-09-01.** The struck sentence describes the state before the index register existed and it is kept because its reasoning is what the build had to respect. `rent_indices` ([RentIndex.php](../app/Models/RentIndex.php), register at `/admin/rent-indices`) holds the published figures, and a lease now states its own clause: **which** index (`escalation_index_code`), the **base figure** the next step is measured from (`escalation_index_base_value`, which rolls forward on each step) and the **publication lag** (`escalation_index_lag_months` — a September figure published in October cannot drive a 1 January step unless the clause says to read three months back). `cpi` is swept alongside the two stated methods ([RentEscalationService.php:71](../app/Services/RentEscalationService.php#L71)); the index move resolves to a percentage and is then **collared** by the lease's own `escalation_floor_rate` / `escalation_ceiling_rate` through `RentEscalationService::collar()`, which is the standard *"greater of CPI or 3%, capped at 10%"* clause. What survives from the struck claim is narrower and is **a stated deviation from Yardi**: there is still no automatic feed — a published statistic is keyed by a person, because inventing an index number is inventing the rent a tenant pays. The refusal is now per lease per day rather than wholesale: a clause naming no index, carrying no base, or whose month has not been published yet is **skipped without rolling the anniversary**, so the sweep lands the step the day the statistic does and a late statistic is never a year the tenant skips. | [01](gap-analysis/README.md) | M |
...
| Item | What & why |
| --- | --- |
| ~~**Owner portal is off**~~ | ✅ **RESOLVED BY REMOVAL 2026-07-27 — this row described a panel that no longer exists.** The operator's call was that owners are admin-panel RBAC users, not a separate portal, so the `/owner` panel and its `owner_portal` feature flag were deleted rather than switched on. `config/features.php` records why at the point where the flag used to be. Anything owner-facing is now an admin screen scoped by ownership tenure — which is also what fixed the former-owner leak, since the tenure-aware scope had to become the only path. |
...
| ~~Dashboard drilldowns~~ | ✅ **CLOSED 2026-08-18 — and one of the three claims was stale.** ETA tiles were already clickable. **AR-ageing** gained a link to `Pages\ArAging` (which carries a bucket Select, so one link is the whole drill-down). **Tenant-mix** now links to the Units list, which already had the category filter. **Top-tenants was NEVER static** — it is a TableWidget with a `recordUrl()` to the lease, and has been all along; pinned by a test so the claim cannot return. Per-SLICE and per-BAR handlers are deliberately not built: they can only be JavaScript on the chart canvas, and nothing here can exercise that in a real browser. |
...
| **Bulk actions** | `bulkSubmitToEta` exists but is unreachable — module 16 is frozen (`Modules::FROZEN`), so in practice there is **no** money-side bulk action; maintenance bulk is delete/restore only. |
...
| ~~**Session encryption + explicit CORS**~~ | ✅ **SHIPPED 2026-08-19.** The row was true and verified before the fix by dumping the resolved config: with no `config/cors.php`, `HandleCors` (which is in the global stack regardless) applied the framework fallback `allowed_origins: ['*']` over `api/*` and `sanctum/csrf-cookie`. **The honest scope, because a fix oversold is a fix nobody trusts:** `/api/v1` is Bearer-token authenticated and `supports_credentials` was false, so the wildcard never handed anyone a tenant's data — a cross-origin call without a token gets a 401. What it did expose was the unauthenticated shopper feed (`/api/v1/public/*`) to any origin, and it meant the policy was an accident rather than a decision. `config/cors.php` now derives the allow-list from `CORS_ALLOWED_ORIGINS`, defaulting to APP_URL. **The native app is unaffected** — CORS is a browser mechanism and a native client sends no `Origin`. Session encryption now defaults ON for any deployed environment via `SecurityDefaults::encryptSessionsByDefault()` (the `force_https` shape); the one-time cost is that everyone signs in again. **`.env.example` pinned `SESSION_ENCRYPT=false`**, which would have overridden the default on every deploy that copies it — the same trap as `LOG_STACK=single` — so it now ships commented out, with a test asserting it. `atriom:health` gained `browser_origin_policy`, which FAILS a deployed box on a wildcard, an empty allow-list, wildcard-plus-credentials, or unencrypted sessions. `BrowserOriginPolicyTest` proves each refusal by feeding it the bad state, paired with a control that must stay green. |

### UI/UX — the daily-use layer (opened 2026-08-23)

**Two rows closed the day this section opened.** The sidebar is now one registry
(`App\Support\Navigation` — see [OVERVIEW §2](OVERVIEW.md#2-architecture-at-a-glance)) and every
optional module has a switch only super_admin may move (see
[modules/18](modules/18-rbac-scoping.md#module-switches--every-optional-module-and-only-the-platform-owner-may-move-one-2026-08-23)).
What is below is what a survey of the panel found next, ordered by how often an operator hits it.
Each row states the evidence, because "the UX could be better" is not a work item.

| # | Item | What & why |
| --- | --- | --- |
| ~~**UX-20**~~ | ~~Filters, sort and search are lost on every navigation~~ — ✅ **THE ROW WAS FALSE, and withdrawn the next day.** It rested on a grep of `app/Filament` finding zero `persistFiltersInSession` calls. The seam is in `app/Support`: `App\Support\TableDefaults` has registered filters, search, column-searches, sort AND the column layout panel-wide through `Table::configureUsing()` all along, plus `striped()` and `reorderableColumns()`. Confirmed by asking a real `ListInvoices` table at runtime rather than by grepping again. **This is the "verify absence claims" rule and I broke it** — an absence found by searching one directory is not an absence. What the investigation DID find is below, and both were real. |
| ~~**UX-20a**~~ | ~~**A remembered search followed the operator across the property switcher**~~ — ✅ **FIXED 2026-08-24.** Filament namespaces the FILTER session key by the Filament tenant (`HasFilters::getTableFiltersSessionKey()` appends the tenant id) and namespaces search, column-search and sort by the component class **alone**. Measured with two properties on one `ListInvoices`: typing `ZARA-ONLY-IN-A` in Mall A and opening Mall B's invoice list re-applied it — the list comes back EMPTY, with a search box the operator has long forgotten they filled. An empty list is the one state that reads as broken rather than as filtered. `App\Listeners\ForgetTableSearchOnPropertyChange` clears the search keys when the property actually CHANGES (it fires on every panel request, so without that guard it would clear the search on the request straight after it was typed). Sort and the column layout are deliberately left to carry across — they mean the same thing in every property and can never empty a list. **The listener is wired by Laravel's auto-discovery**, and a first version added an `Event::listen` beside it that registered the handler twice: harmless at runtime, and it made the mutation test unfalsifiable — deleting the explicit registration left everything green. |
| ~~**UX-20b**~~ | ~~**The "All records" escape did not escape**~~ — ✅ **FIXED 2026-08-24.** A saved view is a URL, and Filament restores a session filter only when the matching property is still empty after the query string binds — so a view that NAMES a filter wins and a view that names none silently does not. Worst for the saved-views menu's "All records" link, whose whole job is to get back to the plain list and which carries `?tableView=none` precisely because an empty query string is indistinguishable from a bare page load. Measured: `?tableView=none` came back still carrying `status = draft`. `SavesTableViews::mountSavesTableViews()` now forgets the remembered filters, search and sort whenever the request names a view — in `mount`, because Livewire runs trait mount hooks before `booted` ones and `bootedInteractsWithTable()` is what reads the session. Both pinned by `RememberedTableStateDoesNotFollowTheOperatorTest`, each refusal paired with a control. |
| ~~**UX-21**~~ | ~~Saved views on 7 of 67 lists~~ — ✅ **DONE 2026-08-24.** Now offered by a RULE rather than by whoever remembered the trait: `App\Support\SavedViews` says a list earns the menu at **three filters or more** (see the class for why three), which is 34 of the 66 lists; 27 gained it. `SavedViewsCoverageConformanceTest` fails the build in BOTH directions — a list that grows a third filter and does not offer views, and one that offers them having dropped below the threshold — so the rule cannot go stale the way a hand-picked list does. |
| ~~**UX-22**~~ | ~~A record page is a wall of tabs~~ — ✅ **ADDRESSED 2026-08-24, and the row's own numbers were wrong.** A first census said Lease had none; it had been taken without an authenticated user, and `getRelations()` is gated. The real figures: **Lease 13 relation managers, Tenant 9, Property 6** — so the wall is worse than the row claimed. What was NOT done is `getRecordSubNavigation()`: converting thirteen relation managers into thirteen pages changes every deep link and every screen-guide mount for a rail. The actual pain the row named — *"it does not tell you which ones have rows"* — is fixed instead by `CountsItsRows`, a tab badge on 15 managers across those three resources. Zero renders **nothing** rather than "0" (a row of grey zeroes carries the same information at far more cost), and every badge is **deferred**, so thirteen COUNTs never block first paint. `RelationManagerTabBadgesMatchTheirRowsTest` drives a real record and fails if a badge disagrees with the rows its tab returns — the failure that matters, since a manager which narrows its own query would otherwise advertise a number the operator cannot reconcile. **That last sentence was FALSE until 2026-09-10 and the gate was vacuous:** it compared the badge against `$owner->{$relationship}()->count()`, which is precisely what the default `badgeCount()` computes — the default measured against itself, agreeing for every manager whatever its table did — and `ownerRecordFor()` handed it EMPTY records, so every count was 0, every badge null and `null === null` passed nineteen times. It stayed green through a live `rows=1 badge=2` defect on a narrowed tab, which is the one thing it exists to catch. It mounts the real table now, seeds owners that have rows, and asserts it saw one. Found by an adversarial review, not by the suite. |
| **UX-23** | **Slide-overs for the Setup catalogues** — **NOT TAKEN, and here is the cost.** The row is factually right: `slideOver()` appears **0 times**, so adding a trade or a failure code takes the operator off the list and back to a re-queried one that has forgotten where they were. But this panel's create/edit are PAGES, not modal actions: a slide-over means moving those resources to `ManageRecords`, which changes their URLs, their deep links, their screen-guide mounting and their tests. That is a structural change to ~19 resources bought for a re-query. Worth doing per-resource if an operator complains about a specific catalogue; not worth a sweep. |
| ~~**UX-24**~~ | ~~The sidebar does not say what is waiting~~ — ✅ **INVERTED 2026-08-24.** The row proposed ADDING badges. Measuring first found the opposite problem: `filament()->getNavigation()` is called by both the sidebar and the topbar blade and `Panel::getNavigation()` resolves a fresh `NavigationManager` each time, so **every badge query ran five times per page render** — ten badge screens, fifty redundant COUNTs on every admin page, 57ms of the 95ms the whole sidebar cost. `App\Support\Filament\NavigationItemMemo` (bound `scoped`, never `singleton` — a queue worker outlives the request) memoises each screen's navigation items for the request. Measured on demo data: invoices **130 → 86** queries, leases **408 → 320**, work orders **243 → 111**. Adding badges is now affordable and is the follow-up; the count that was blocking it is gone. |
| **UX-25** | **Deferred loading** — **STILL NOT TAKEN, and less needed than it was.** `deferLoading()` remains absent. The case for it was the query cost of a list, and that has now moved twice: the navigation memo took invoices 130 → 86 and work orders 243 → 111, and eager-loading the deposit relations took **leases 408 → 86**. Deferring would still buy first paint, at the price of a second round trip on every list view and of breaking every test that reads rows through the shared `tableRows()` helper. Take it per-table if a specific list proves slow in a browser. |
| ~~**UX-26**~~ | ~~The technician's screens on a phone~~ — ✅ **NARROWED 2026-08-24; the LOOK still needs your eyes.** Work orders showed **17 columns** by default and tenant requests **15** — on a 375px screen a horizontal scroll through information a technician standing at a chiller cannot act on (which property, who bears the cost, whether it is over the NTE limit). `Column::visibleFrom('md')` holds the desk-work columns back below 768px: **17 → 6 on a phone, unchanged at a desk**. Chosen over `isToggledHiddenByDefault` deliberately — hiding them by default would take them from the SUPERVISOR, who is the other user of the same list and the one those columns were added for. Measured: the page still builds 19 columns, still renders 19 `<th>`, still costs 47 queries; the cells are CSS-hidden, not omitted, so sorting, filtering, the toggle menu and exports are untouched. `TheTechniciansScreensFitAPhoneTest` pins which six survive and that the class reaches the DOM. **What no PHP test can answer is whether it LOOKS right** — `tests/e2e/24-phone-technician.spec.js` is that check, at an iPhone 13 viewport, and is deliberately not in any automated run (the browser suite is advisory, CI is paused, and E2E writes to the dev database). Run it, or just open the two screens on a handset. |
| ~~**UX-27**~~ | ~~Field help is sorted but not finished~~ — ✅ **MEASURED 2026-08-24, and the answer is that it very nearly IS finished.** A first pass built all 66 create forms and asked each field for its helper text, reporting "11% of 673 fields carry help". That number was worthless: `Field::getHelperText()` **throws for every field outside a mounted Livewire container** — all 673 of them — so the run measured nothing and would have gone on saying so. Re-done as a SOURCE sweep, the real picture is that of every field carrying a limit the form enforces, exactly **six** did not state it, and five of those six are self-evident (a percentage capped at 100, a display-order ceiling, a duration that must be at least one). One was a genuine policy nobody could guess and now says so. `BoundedFieldsExplainTheirLimitConformanceTest` holds the line in both directions: a new bound must be explained or registered in `FieldHelp::SELF_EVIDENT_BOUNDS` with a reason, and an exemption whose field later gains help is reported as stale. |
| ~~**UX-28**~~ | ~~One keyboard shortcut exists, and it is the only one~~ — ✅ **FALSE, and fixed differently 2026-08-24.** Filament v4 already binds `mod+s` to Save on every Edit page and Create on every Create page, and `mod+shift+s` to "Create & create another"; this panel adds `⌘K`. A first check searched the rendered page for the literal `mod+s` and found nothing — Filament emits `x-mousetrap.global.mod-s`. So the gap was never the bindings, it was that only `⌘K` advertises itself, because the search field prints its own binding. `KeyboardShortcutsAction` puts the list in the user menu of BOTH panels, and `KeyboardShortcutsAreAdvertisedTest` refuses to let it teach a key the panel does not emit — which it immediately caught, since the search binding is `command+k`, not the `mod+k` the first list claimed. |
| ~~**UX-29**~~ | ~~Arabic is translated but the panel has never been walked in RTL~~ — ✅ **AUDITED 2026-08-24; structurally clean.** Zero physical-direction Tailwind classes in any blade; every PDF template already branches on `$isRtl` for its borders. Rendering the dashboard, a list, a record edit and Settings with the user's locale set to `ar` gives `dir="rtl"`, Arabic content and **no leftover translation keys** on any of them. (A first attempt called `app()->setLocale('ar')` before dispatching the request and reported every page as LTR — `SetLocale` runs in the middleware stack and had already resolved the locale from the user.) What genuinely remains is visual: a browser at width, which no PHP test can stand in for. |
| ~~**UX-30**~~ | ~~**A document's LINES are still frozen in the writer's language**~~ **SHIPPED 2026-09-06.** `invoice_items` and `credit_note_items` carry `description_key` + `description_data`, resolved by `App\Support\LineNarrative` when the document is read — the PDFs, the portal, the mobile API and the forecast all go through it, and `description` stays as the floor. Nine writers converted; the whole sentence is ONE template per key, because Arabic does not put a parenthetical where English does, and a DATE or a CLASSIFICATION inside the line is stored as data and worded for the reader too — the one-sentence-in-two-languages trap `LeaseEventNarrative` had to be shown on screen. An operator who words a line themselves keeps their words: typing in the description clears the key, on the model so no writer can forget. `LineNarrativeIsAKeyNotProseConformanceTest` gates it; six mutations, and one of them caught the regression test asserting everything about the line EXCEPT the classification it was named for. **`credit_notes.reason_notes` closed it out 2026-09-06** — the paragraph a tenant reads ABOVE the lines, raw English from the CAM sweep and a write-time `__()` from the move-out one, so a single credit note carried an English explanation over Arabic line text. The trait generalised on that second real call site (`DescribesItsLine` → `WordsItselfForItsReader`, a column MAP rather than a copy) because a credit note is not a line. **The first pass shipped the credit-note half INERT and said otherwise** — the columns existed and both readers called `narrative()`, but `CreditNote::describeAs()`, the one seam that creates a credit-note line, had nowhere to put a key, so no writer could store one; the review caught the claim before anyone relied on it. *(Original row below.)* | **A document's LINES are still frozen in the writer's language** — **opened 2026-08-31 with evidence.** `DocumentLocale` (2026-08-27) made every PDF render in its READER's language, and `JournalNarrative` / `LeaseEventNarrative` made a stored row keep DATA rather than prose. **What neither reached is the line text on the money documents themselves.** Measured on a real CAM credit note: the PDF renders «تسوية» for `reason` (a key, resolved at read time ✅) directly above `CAM reconciliation credit — 2026` for `reason_notes` and a line reading `CAM over-recovery returned — 2026 reconciliation` — both composed with `__()` at WRITE time and stored, so they freeze in whichever language the operator happened to be running, on the document a tenant files with their own accountant. **The Arabic translation of that line already exists and can never reach the tenant** unless the operator was in Arabic when they billed. **Not a CAM defect — a systemic one**: `MonthlyBillingService` appends a raw-English `' (75% pro-rated)'` to every pro-rated line on every monthly invoice in the portfolio, which was never translatable at all. The fix is the shape the other two took — a key column plus a read-time resolver over `invoice_items` / `credit_notes` — and it is EG-36-sized, which is why it is a row and not a patch. **Do NOT half-do it**: resolving in the recipient's language at write time would make it read as fixed while a tenant who switches language still gets the old one, and a screen that looks correct and is not is this project's most expensive failure mode. |
...
- ✅ **"ETA receiver address per tenant" + "ETA retry policy is untested"** — **both done
  ...
  **[modules/16](modules/16-eta-einvoicing.md)** — which is now **🧊 FROZEN (2026-08-22)**: off in code via `Modules::FROZEN` and shown nowhere, services and tests kept.
...
> **The single go-live gate is [STATUS.md](STATUS.md)** — every configuration item, credential and
...
2. **The remaining ⚙️ go-live rows** (§2) — backups are now half done in-app (2026-07-29);
   what is left there is off-site copies, an archive password and **a tested restore**.
   ~~There is still no deploy workflow at all, which makes several other rows moot.~~
   **Corrected 2026-08-23:** `./deploy.sh` has existed since 2026-08-19 — the claim survived four
   days past its own falsification and was repeated as fact when someone asked what was next. The
   real finding on re-reading it was narrower and worse: the script ran the nine commands in
   PRODUCTION-RUNBOOK §2's code block and **skipped the two the prose beside that block calls
   REQUIRED** — `atriom:install --force` (a new catalogue arrives empty; a new permission hides its
   screen from everyone, including super_admin) and `atriom:rebuild-search` (every existing row keeps
   the previous release's fold and the search bar says the record does not exist). Both now run, and
   `DeployScriptRunsTheReleaseSequenceTest` reads the script's EXECUTABLE lines — not its comments,
   which mention several of these steps in prose — so a step cannot quietly leave again.
...
| ~~**TX-08**~~ | ✅ | 🧑‍💻 | **SHIPPED 2026-08-19.** Stamp and schedule tax are commissioned: a fresh install ships them ON, an existing DB gets the posting role backfilled and stays OFF (a reseed must never reactivate a code the operator retired). **The accounts were never the real blocker** — all three journalizers threw the document's own `tax_code` away (the invoice one summed every line's tax into one accumulator and credited `vat_payable`; the vendor-bill and expense ones hard-coded `vat_recoverable`), so an active stamp code would have put 20% of a supply on the VAT return, under the VAT liability, balanced and tied out. Tax now groups by the tax code's own posting role, exactly as revenue groups by the charge code's. **The asymmetry is the content and must not be flattened:** output stamp/schedule are LIABILITIES, input stamp/schedule are EXPENSES (`stamp_tax_expense` / `schedule_tax_expense`), because neither has a credit mechanism the way input VAT does — copying the VAT shape there grows a receivable nobody can ever collect. |
...
| **CFG-05** | ✅ | 🧑‍💻 | **SHIPPED 2026-08-12.** `/admin/configuration-health` — seven checks that read the LIVE database (payroll added 2026-08-20, EG-04), each saying **what the gap breaks** rather than which field is empty ("tenants cannot reclaim the VAT you charged them", not "seller_tax_registration_number is empty"). `atriom:health` answers *is it alive*; this answers *is it set up*, and the two fail differently — a perfectly healthy install bills through a floor rate because nobody classified the charge codes. Severity is about money and law, not tidiness. Every check is tested from both sides: a checklist that reports all-clear because its detection is broken is a green light nobody earned. **On the GATE since 2026-08-25**, not only on a screen: `atriom:config-health` runs the eight from the CLI and `atriom:preflight` runs it second — until then a release could pass every automated gate with no seller TRN, an incomplete posting map and no open period, because the only way to run these was to open a page. Blocking rows decide the exit code; `--strict` adds the advisories. *(Original row below.)* **Configuration health page** — what is unset and what it breaks; the in-app form of [STATUS.md](STATUS.md), and the analogue of Yardi's setup checklist. `atriom:health` already computes most of it. |
| **CFG-06** | ✅ | 🧑‍💻 | **SHIPPED 2026-08-12.** `SettingsReachConformanceTest` over `App\Support\SettingsReach`. **Two halves, because a name-grep is not enough.** Half one: every public property of every settings class must be read somewhere outside `app/Settings` and the settings page — catches the setting nothing consults. Half two is the one that matters: every `?? <settings read>` over a **NOT NULL** column is refused unless declared, because that fallback is dead code and the setting silently applies to nothing. That is precisely the shape CFG-03 found by hand — `default_payment_terms_days` had a screen AND a reader and was still inert at eight billing call sites. Both halves were mutation-tested; half two names the offending file and line. **It found a third live inert setting:** `eta.issuer_name` and `eta.issuer_tax_registration_number` were `->required()` on the settings page and read by **nothing**; parked in `KNOWN_INERT` — deliberately a separate list from `EXEMPT_NO_READER`, since "this is correct" and "this is broken and here is why we have not fixed it" are different claims, and collapsing them is how a real gap becomes invisible again. **Closed 2026-08-22:** the whole `eta.*` group (all four properties, not two) was DELETED with the module freeze rather than given a reader — retiring the setting is the fix a reader would have hidden. `KNOWN_INERT` is now empty, and its two checks were rewritten to assert the empty case rather than loop over nothing. *(Original row below.)* **Write-side/read-side conformance test** — every setting a screen writes must be the one the code reads. The inert-screen trap becomes a gate instead of a memory. |
...
| **RP-04** | ✅ | 🧑‍💻 | **SHIPPED 2026-08-12.** A saved view carries a schedule (monthly/weekly, day, recipients) and `reports:deliver` — on the scheduler at 06:00 — emails it as CSV. **It renders AS the person who saved it**, because a report reads whatever the current user may read and a console command has no current user; their `canAccess()` is re-checked, so a schedule stops when access is withdrawn rather than being a standing grant. Idempotent: `last_delivered_on` is claimed under a lock inside the transaction. A report is deliverable when it implements `DeliverableReport` — i.e. when it has a CSV at all. The rest are a checklist, a floor plan, a diagram, a dry run and a PDF pack: they have no CSV to send, which is a better reason than "nobody lifted it yet". `ReportCatalogue::NOT_DELIVERABLE` names each with its reason, gated so a new report must declare which it is. *(Counts were hand-typed here and in module 17 and had drifted in both; one of the pages named as undeliverable — "a searchable log" — is the audit log, which is deliverable and as of 2026-08-22 has the export button too.)* *(Original row below.)* **Scheduled delivery** — email a saved report on a schedule (the month-end pack to the owner). Nothing exists today; the scheduler, the PDF builders and the CSV exporter all do. |
...
| **RP-06** | ✅ | 🧑‍💻 | **COMPLETE 2026-08-18.** The comparative half shipped 2026-08-12 (prior period · prior year). **Budget-vs-actual now ships too:** a `budget_lines` table (per property, per account, per MONTH — a mall is seasonal, so an annual figure over twelve would report a variance that is really just the season), a paste importer on `Pages\Budget`, and `BUDGET` as a third basis on `ComparativeStatementService`. The trick that kept it small: `BudgetService::asIncomeStatement()` returns the SAME shape a prior period does, so the change column, the percentages, the totals and the CSV all work unmodified. Importing **replaces** the year — an account dropped from a revision is removed, not left at its old figure. `BudgetVsActualTest`. |
...
| **RP-09** | ✅ | 🧑‍💻 | **SHIPPED 2026-09-12 — the reports audit, four commits (`eae08cef` · `c083a12e` · `3e1feb79` · `59b623b1`).** Every catalogued report and the twelve accounting registers were rendered on the demo books as the `accounting` role in both languages and read against the reference systems' standard set; all 26 render, no raw key, no Arabic-Indic digit, no other system named. **Four defects fixed, each through `/safe-change` with an adversarial review that found something real in every one:** (1) a financial statement's chart-group SUBTOTAL printed with no HEADING above it — the demo income statement read *"Total Operating Revenue 10,345,002.01 … Total operating revenue 10,350,942.01"* three rows apart — on screen, CSV, PDF and both alternative readings, and the 12-month spread's PDF printed no group lines at all (`StatementGroups::headingFor()`, one resolver, five renderers; the spread's layout lives once in `StatementSpread::records()`); (2) **there was no aged-payables report** — the one standard financial report the catalogue had no counterpart to — `/admin/ap-aging`, per supplier, aged by due date on the receivables' own buckets, ✓/⚠ against the AP control account through `glTieOut()` itself, scoped to the mall on screen, its own **Payables** hub heading; (3) the general ledger read ONE account and had no PDF — "Every account" now, each under its heading with opening and closing, saved-view and delivery aware, printed on one template (and the review found the saved-view URL not read at mount, a delivery-path window on older views, no `resetPage()`, an Arabic PDF printing `-338,003.70` as `338,003.70-`, and 677 document loads per render for 50 rows); (4) the journal register printed a MORPH ALIAS as the source of seven posting kinds (`SourceDocumentLabel`). Details: [modules/17](modules/17-reports.md#aged-payables-2026-09-12-the-reports-audit) · [modules/21](modules/21-general-ledger.md#the-general-ledger-reads-every-account-and-prints-2026-09-12-the-reports-audit). |
| **RP-10** | 🟡 | 🧑‍💻 | **The rest of the audit — measured, not built.** (a) **Balance sheet: one line *"Net income (not yet closed)"* lumps every unclosed year** — on the demo (2024 + 2025 unclosed) it disagrees with the income statement's net profit beside it by 135,771.76, and on a live install it does so for the first months of every year until the close is run; the reference shape is *Retained earnings (prior years)* + *Current-year earnings*, two windows of `aggregate()`. Ask the accountant how prior unclosed years should read before building. (b) **No export on the accountant's registers** — Journal Entries, Chart of Accounts (importable, not exportable), Vendor Bills, Expenses, Security Deposits, Post-dated Cheques; Payments/Invoices/Credit Notes have one. Each is an exporter class + `IdentifiedExportAction` under the `Exports` gate; the chart's must pair with `LedgerAccountImporter`'s labels in both locales (the round-trip gate). (c) **AR Aging — Detail is bucket-only** (defaults to 1–30, no "every bucket" invoice listing); the collections worklist and the tenant statement cover the question. (d) AR/AP ageing have CSV/XLSX and no PDF. (e) `docs/accounting/README.md` §3–§5 are stale (test count, opening balances listed as not built). |
