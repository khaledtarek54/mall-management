...
| # | Item | What & why |
| --- | --- | --- |
| **UX-20** | **Filters, sort and search are lost on every navigation** | Measured: **zero** occurrences of `persistFiltersInSession` / `persistSortInSession` / `persistSearchInSession` across all 67 list pages. A collections clerk filters Invoices to *overdue, this property, > 30 days*, opens one invoice, comes back — and starts again. This is the single highest-frequency friction in the panel and it is one call per table. Do it in the shared `Tables` classes, not per resource. **Interaction with saved views:** `SavesTableViews` stores a NAMED state deliberately; session persistence is the unnamed one. They compose — the view sets the state, the session remembers where you left it — but the "All records" reset (`?tableView=none`) must clear the session too, or the escape hatch stops escaping. |
| **UX-21** | **Saved views on 7 of 67 lists** | `SavesTableViews` (EG-32 slice 1, UX-11) is built, tested, carries a personal-vs-team default and rides as `?tableView={id}`. It is mounted on Invoices, Payments, Leases, Tenants, Units, Work orders and Requests. The other sixty lists — vendor bills, expenses, journal entries, violations, work permits, the whole Setup group — have none. The work is mounting a trait, not building anything. |
| **UX-22** | **A record page is a wall of tabs; Filament has a left rail for it** | `getRecordSubNavigation()` is used **0 times**. A Lease record carries charges, invoices, documents, events, options, units and notes as relation-manager tabs across the top, which stops being scannable past about five and does not tell you which ones have rows. Sub-navigation turns them into a labelled rail with counts. Highest value on Lease, Tenant, Work order, Vendor and Invoice. |
| **UX-23** | **Every create and edit is a full-page navigation, including for a two-field catalogue row** | `slideOver()` is used **0 times**. Adding a trade, a failure code, a holiday or a violation category takes the operator off the list, and back to a re-queried list that has forgotten where they were. A slide-over keeps the list on screen. Scope this to the **Setup** group only — a money document deserves a full page and its own URL. |
| **UX-24** | **The sidebar does not say what is waiting** | 23 screens define `getNavigationBadge()`, which is a good start and an inconsistent one. The worklists that should carry a count and do not are the ones an operator opens *because* something is in them: overdue invoices, SLA-breached requests and work orders, approvals pending my tier, post-dated cheques maturing this week, expiring vendor certificates. **Badges cost a COUNT per render** on every page, so they belong on the handful of screens where the number changes the operator's next action — not on all 99. |
| **UX-25** | **Heavy tables block first paint** | `deferLoading()` is used **0 times**. Invoices, journal entries and the general ledger render their first page synchronously with the layout. Deferring lets the chrome paint immediately and the rows arrive after, which on a slow connection is the difference between "loading" and "broken". Cheap, per-table, and reversible. |
| **UX-26** | **The technician's screens have never been checked on a phone** | The operator's decision (O3, DECLINED) is that technicians use the admin panel rather than a dedicated app, which makes the `technician` role's experience a requirement rather than a nicety. Nobody has driven Work orders → labour → parts → completion evidence on a real handset. Playwright can do this at a phone viewport; the manifest-driven smoke suite already knows every screen. |
| **UX-27** | **Field help is sorted but not finished** | `FieldHelpConformanceTest` polices the 18-word budget and the three homes (`helperText` / `hintIcon` / screen guide), and `ScreenGuides` covers all 110 screens in both languages. What has never been measured is COVERAGE: how many fields that carry a real constraint have no help at all. The gate checks the help that exists, not the help that is missing — the same shape as every "a gate can report on a set it has silently stopped collecting" note in CLAUDE.md. |
| **UX-28** | **One keyboard shortcut exists, and it is the only one** | ⌘K opens global search and renders its own binding in the field, which is why anyone knows about it. Nothing else is bound. The candidates are the ones a keyboard-first accountant would want: save (⌘S) on a form, new-record on a list, and the property switcher. Filament v4 supports action key bindings; the discoverability lesson from ⌘K is that a binding nobody can see does not exist. |
| **UX-29** | **Arabic is translated but the panel has never been walked in RTL** | Both languages are gated for completeness (`TranslationKeyConformanceTest`, and every parity check passes `fallback: false` because `Lang::has()` falls back to English). The visual handbook is RTL-clean with no plugin because it uses CSS logical properties. The admin panel relies on Filament's own RTL, and no one has looked at the money forms, the repeaters, the charts or the PDFs in Arabic at width. |
