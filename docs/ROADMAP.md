...
| **RP-10** | 🟡 | 🧑‍💻 | **The rest of the audit — measured, not built.** (a) **Balance sheet: one line *"Net income (not yet closed)"* lumps every unclosed year** — on the demo (2024 + 2025 unclosed) it disagrees with the income statement's net profit beside it by 135,771.76, and on a live install it does so for the first months of every year until the close is run; the reference shape is *Retained earnings (prior years)* + *Current-year earnings*, two windows of `aggregate()`. Ask the accountant how prior unclosed years should read before building. (b) ✅ **SHIPPED 2026-09-13** — export on Journal Entries, Chart of Accounts, Vendor Bills, Expenses, Security Deposits and Post-dated Cheques, under the `Exports` gate; the chart's file round-trips through its importer ([modules/17](modules/17-reports.md#an-export-says-which-rows-it-is-about)). (c) **AR Aging — Detail is bucket-only** (defaults to 1–30, no "every bucket" invoice listing); the collections worklist and the tenant statement cover the question. (d) AR/AP ageing have CSV/XLSX and no PDF. (e) `docs/accounting/README.md` §3–§5 are stale (test count, opening balances listed as not built). |
