...
| **Production env defaults** | 🟡 **Timezone done 2026-07-29 — and it was a MONEY bug, not a scheduling one.** `app.timezone` is what `now()` returns, so it decides which day and which accounting period a document belongs to. Under UTC every document created between ~21:00 and midnight Cairo was attributed to the PREVIOUS day: a payment taken at 00:30 Cairo on 1 Aug was stored as `2026-07-31 21:30`, putting its `payment_date`, its GL `entry_date` and its period in **July** — three hours every day, on a system whose invariants are all period attribution. The default is now `Africa/Cairo` rather than an env var production had to remember (a fail-OPEN default: forget it, get wrong books); the suite pins UTC in `phpunit.xml` so its determinism is a stated choice, not a side effect. ~20 scheduled jobs declare wall-clock times with no `->timezone()`, so they now mean Cairo — what an operator reading `routes/console.php` assumes. Guarded by `ApplicationTimezoneTest`. **Still open:** `LOG_LEVEL=debug` ships (leaks SQL/PII, fills disk) and there is no logrotate guidance. | ⚙️ | S |
