...
| The six dates (sign / possession / rent commencement / term commencement / expiry / move-out) | ✅ | ✅ **CLOSED 2026-08-10.** `possession_date` + `rent_commencement_date` added; `fit_out_months` **dropped** rather than kept beside them, with an exact PHP backfill so no lease changed what it bills. Term commencement is `commencement_date`, expiry is `expiry_date`, move-out is the termination event. Sign date is the one still unmodelled, and it drives nothing | ✅ **KEEP** | ⚪ |
| Area as a first-class, date-ranged number | ✅ drives rent, recoveries, breakpoints | **ROW CORRECTED 2026-08-10 — it was wrong on both counts.** Area is **not static**: `Lease::totalAreaSqmOn($date)` and `totalAreaSqmForPeriod($start,$end)` date-range it through the `lease_unit` pivot, and the period form is **time-weighted** — a tenant who took 300 m² on 1 November carries CAM for the days held, which is precisely the Yardi re-basing this row asked for. Nor is it **used only by CAM**: `CamReconciliationService` uses it for both the per-lease share and the denominator, `LeaseSpaceChangeService` writes dated expansions/contractions, and the rent roll, reports and rate-priced rent all read the as-at form. **Narrow residual:** a UNIT's own measurement (`units.area_sqm`) is not versioned, so remeasuring a unit silently shifts every historical figure recomputed from it. That is a small, self-contained build — a dated `unit_areas` register read by `totalAreaSqmOn` — not the wholesale refactor this row implied | ➕ EXTEND — version the unit measurement only | 🟡 |
...
| Charge codes as configuration (code → GL account, tax, recoverable, deposit) | ✅ an accountant adds one | ✅ **CLOSED 2026-08-10.** `charge_codes` is the catalogue (code, bilingual label, posting role, active, order) with its own screen; the journalizer resolves the role from it, so adding "key money" is a row rather than a deploy. The role resolves through `account_mappings`, so a new code inherits the per-property override. Behaviour stays in code: the codes the engine has logic for remain `InvoiceItemType` constants and the gate asserts the catalogue covers every one | ✅ **KEEP** | ⚪ |
| Escalation: stored | ✅ %, amount, index, **floor/ceiling**, compounding | **Collar CLOSED 2026-08-10.** `escalation_floor_rate` / `escalation_ceiling_rate` clamp whatever rate the sweep is about to apply (`RentEscalationService::collar()`), so they bite before CPI exists — on a `fixed_percent` lease the ceiling is a rail against a mistyped rate. A floor above the ceiling is refused at the model. Fixed-**amount** escalation CLOSED the same day (`escalation_amount` + `fixed_amount` type, swept on the same anniversary path). Compounding stays implicit — a percentage step multiplies the current rent, an amount step adds to it, which is what each of those terms means | ✅ **KEEP** | ⚪ |
...
| Termination | ✅ with a final account | ✅ **ROW STALE — MF-02/MF-03 shipped.** `MoveOutStatementService` draws the final account (and names the figures that are not knowable yet, rather than quietly omitting them); `SettleMoveOutService` disposes of the deposit in one act and **freezes** the account. Charge deactivation and the fully-unpaid-only invoice cancellation were already correct | ✅ **KEEP** | ⚪ |
...
| On-account / open credit | ✅ auto-applies to the next charge | ✅ **CLOSED 2026-08-11.** `TenantCreditApplication` is its own dated GL document (`Dr Unearned / Cr AR`), and it now applies AUTOMATICALLY when an invoice is issued — Voyager's behaviour, adopted as the default. Configurable (`billing.auto_apply_tenant_credit`) because a credit raised in dispute, or one the tenant expects refunded in cash, should not silently disappear into next month's rent; Voyager makes it configurable for the same reason | ✅ **KEEP** | ⚪ |
