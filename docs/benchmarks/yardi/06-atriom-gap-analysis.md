...
| Area as a first-class, date-ranged number | ✅ drives rent, recoveries, breakpoints | **ROW CORRECTED 2026-08-10 — it was wrong on both counts.** Area is **not static**: `Lease::totalAreaSqmOn($date)` and `totalAreaSqmForPeriod($start,$end)` date-range it through the `lease_unit` pivot, and the period form is **time-weighted** — a tenant who took 300 m² on 1 November carries CAM for the days held, which is precisely the Yardi re-basing this row asked for. Nor is it **used only by CAM**: `CamReconciliationService` uses it for both the per-lease share and the denominator, `LeaseSpaceChangeService` writes dated expansions/contractions, and the rent roll, reports and rate-priced rent all read the as-at form. **Residual CLOSED 2026-08-11:** `unit_areas` versions a unit's own measurement, `Unit::areaOn($date)` answers as-at, and `totalAreaSqmOn()` reads it — so remeasuring a shop no longer shifts the area a past period was billed on. `RemeasureUnitService` closes the row in force and opens the next, exactly as the charge schedule does for money | ✅ **KEEP** | ⚪ |
