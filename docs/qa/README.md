...
| **The tester's guide** — [docs/visual/testing/](../visual/testing/index.md) | Lives in the **handbook**, so a tester reads it *on the box* at `/admin/handbook`. Three pages: how to test (access, dataset, bug format, what NOT to test), one ordered money cycle with the figure to expect at each step, and every module with what it is *for* in business terms. **Re-verified against the live box 2026-09-05.** |
| **[tester-pack/](tester-pack/README.md)** — the same material, self-contained | For a tester working **with an AI assistant**. Plain Markdown, no links out to the handbook site, so the whole folder can be handed to Claude and asked *"is this a bug?"*. Adds a **system explanation** (the money spine, the four settlement channels, VAT, proration, what is deliberately true and looks like a bug) and a **glossary** that the three handbook pages assume you got elsewhere. Keep the two in step — they describe one box. |
...
| [DEEP-SWEEP-2026-09-01.md](DEEP-SWEEP-2026-09-01.md) | The **post-staging deep sweep** (2026-09-01) — 23 finder agents over the money/GL seams and both portals plus every admin panel, each finding then adjudicated by a second agent told to refute it. **204 open, listed with file and line**; four were fixed the same day (three criticals) and are recorded at the top. The worklist is [ROADMAP §9](../ROADMAP.md#9--cycle--the-2026-09-01-deep-sweep-sw). |
| [STAGING-SOAK-2026-09.md](STAGING-SOAK-2026-09.md) | The **month-long staging soak** (from 2026-09-05) — a second property, **Nile Gate Mall (NG)**, seeded by `NileGateSeeder` beside the Val Plaza demo mall and left to the scheduler. Carries the calendar of what should happen on which day, the acts a person performs during the month, the daily-check procedure (`scripts/soak-check.sh`, cron on the box, posts to Discord) and the day-by-day journal. |
...
2. **Handing the panel to a tester**, point them at `/admin/handbook` → **Testing the system**, and nothing else — it carries its own access, dataset and script. It is deliberately a CYCLE, not a per-screen matrix: see the note below on why the per-module matrix was deleted. If they are working **with an AI assistant**, give them [`tester-pack/`](tester-pack/README.md) instead — same material, self-contained, plus the system explanation a model needs to judge whether a number is right.
   > **While the [soak](STAGING-SOAK-2026-09.md) is running, a tester must NOT reset the box and must NOT write to Nile Gate (NG).** Both pages say so at the top. Val Plaza (VP) is the sandbox — note it has no bank account, so the tester creates one first.
...
4. Keep it **versioned** — these are living documents; update them in the same commit as the feature they cover (docs are part of "done").
