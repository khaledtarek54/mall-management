...
| **D3-04** | ~~`TableView::makeDefault()` clears defaults by the **view owner's** id, so a colleague adopting a shared view wipes the owner's personal default~~ **FIXED 2026-09-05** — the clearing is scoped to the ACTOR, plus the shared tier when the marked view is itself shared (two team defaults resolve by row id, which nobody decided). Mutation-proved per tooth. **STILL OPEN, and an adversarial review the same day showed it is the LARGER half:** `is_default` is one column answering two questions, so a view its owner shared AND marked is both their personal default and the team's — any clearing of "other shared defaults" therefore wipes a preference somebody stated (measured; the first version of the fix did exactly that and its own test pinned it as correct). Adoption also writes the flag on somebody else's row, so one click moves every colleague's landing screen, and a non-owner's "clear" cannot escape it — the flag is a column on the shared row, so there is nowhere to record "not for me". Needs the per-user pivot `ReportPreference` already models. | A preference, not money | S remaining |
