...
| **AR-GL-03** | The tenant statement itemizes two of the four settlement channels its own `total_paid` counts — worst on final move-out statements | Figures are correct; only the itemization cannot be reconciled by eye | S |
...
| **D2-09** | **No retention/prune** for `notifications`, `exports` (+files), `failed_import_rows`, `failed_jobs`, expired Sanctum tokens — only activity_log got EG-34's treatment | Years of growth, not weeks | S |
...
| **GAP1B-02** | **Consolidated financial statements** | The code supports consolidation and no surface reaches it, while STATUS §4 claims it ships "as described". Either reopen the All-Properties decision (M) or fix the STATUS row (XS) — but decide |
