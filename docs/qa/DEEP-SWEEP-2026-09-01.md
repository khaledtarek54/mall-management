...
> ### Where this stands — 25 closed, 192 open (updated 2026-09-01)
>
...
*23 open — 2 high, 12 medium, 9 low.*
...
| **SW-003b** | open | medium | S | *(found by the review of SW-003.)* `PaymentForm` caps each allocation ROW independently while `CreatePayment::afterCreate()` SUMS rows against the same invoice, and the repeater has no `->distinct()` on `invoice_id` — so 700 + 600 on a 1,000 invoice passes every form gate and is refused only at the model. The refusal is correct; the UX is not, and it is an ordinary operator slip (`PaymentFormGuardsTest` already treats duplicate rows as supported input) | `Filament/Admin/Resources/Payments/Schemas/PaymentForm:241` |
