...
| **SW-039** | ✅ fixed | high | S | Writing off a partially-paid billed deposit erases the PAID portion from depositHeld — tenant's deposit silently kept at move-out | `Models/Lease:674` |
| **SW-040** | ✅ fixed | high | — | An assessment invoice records the calendar month it did not bill, so a resale credits the seller too little and the mall over-collects | `Services/BillUnitOwnershipsService:268` |
...
| **SW-039b** | ✅ fixed | high | S | *(found by the review of SW-039.)* A PARTIAL credit note inflated the deposit pot: `credit_applied_amount` feeds `paid_amount`, so crediting 40,000 of a 100,000 deposit invoice with 60,000 of cash on it left the invoice `paid` — never `credited` — and a move-out refunded 40,000 that never arrived. Outbound, with no recovery path | `Support/DepositBilling:heldOn` |
| **SW-039c** | ✅ fixed | high | S | *(found by the review of SW-039.)* A PARTIAL write-off made the deposit shortfall permanently un-re-billable — `WriteOffInvoiceService` retires an invoice only on a FULL write-off, so no status moves and the forgiven slice went on counting as *already asked for*. The **Bill deposit** button stayed hidden and the service refused quoting money the operator had themselves forgiven | `Support/DepositBilling:claimedOn` |
| **SW-039d** | ✅ fixed | high | S | *(found by the review of SW-039.)* `DepositHoldings` carried the same status filter as two LITERALS — a third and fourth copy — so the register header read *"held 0.00"* under a red GL-gap stat while the lease page beside it read 60,000 | `Support/DepositHoldings:83` |
