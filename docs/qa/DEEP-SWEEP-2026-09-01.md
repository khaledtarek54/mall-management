...
> ### Where this stands — 24 closed, 193 open (updated 2026-09-01)
>
...
*24 open — 2 high, 13 medium, 9 low.*
...
| **SW-003d** | open | medium | M | *(found by the review of SW-003.)* **No panel opts into Filament's database transactions** (`HasDatabaseTransactions` defaults false, nothing calls `databaseTransactions()`), so `beginDatabaseTransaction()`/`rollBack` are no-ops app-wide and EVERY Create/Edit page that throws in `afterCreate`/`afterSave` commits its record. Named instances: `EditPayment::afterSave()` rolls back the allocations while the header edit stands, and `CreateLease::afterCreate()` can leave a committed lease with a partial or empty charge ladder — the *"leases whose charge rows bill NOTHING"* state `atriom:audit-charge-schedules` exists to find | `Providers/Filament/AdminPanelProvider` |
