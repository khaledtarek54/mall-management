...
**Shared (28)** — portfolio-wide by design:

`AccountMapping` · `AccountingPeriod` · `ApprovalRule` · `ChargeCode` · `DeviceToken` · `FailureCode` · `FiscalYear` · `InventoryItem` · `LedgerAccount` · `Note` · `PaymentMethod` · `RentIndex` · `ReportPreference` · `SavedReport` · `SystemSetting` · `TableView` · `TaxCode` · `TaxRate` · `TenantDocument` · `TenantUser` · `Tenant` · `Trade` · `User` · `UtilityTariffRate` · `UtilityTariff` · `VendorContact` · `VendorDocument` · `Vendor`
