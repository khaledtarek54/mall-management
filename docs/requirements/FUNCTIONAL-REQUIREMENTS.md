...
> Status: 🟢 v3 — most requirement groups built & shipped; **live build status in §3**, hands-on checks in [qa/UAT-SCRIPTS.md](../qa/UAT-SCRIPTS.md)
...
> **This table + the per-section commits are the authoritative state.** The per-FR tags in §§4–14 below reflect the *original plan* (planning-time), not live status — read them as design intent. To validate the built features hands-on, see [qa/UAT-SCRIPTS.md](../qa/UAT-SCRIPTS.md).
...
1. **#4 — overdue → owners + late fees.** (a) *Small:* add owner (Jawad) users to the recipient set of `TenantRequestSlaBreachedNotification` (the daily `requests:scan-sla-breaches` job already fires it to staff). (b) *Needs your input:* late fees — decide **O-3** (what triggers a fee — past the work-window end, or the SLA deadline?) and **O-4** (who is charged + the amount), then realize it as a charge + alert.
...
- **REQ-1** 🟢 `[DONE]` — Every request carries a **scheduled work window**: `from` date/time → `to` date/time (when the work is performed). **Implemented:** `scheduled_from` / `scheduled_to` on `tenant_requests` and `owner_requests`, exposed in the forms. — *covers original request #6.*
- **REQ-2** 🟢 `[EXISTS]` — Status lifecycle defined for maintenance: `submitted → acknowledged → in_progress → awaiting_tenant → resolved → closed`, plus `cancelled`. Other request types reuse a comparable lifecycle. See [`STATUSES`](../../app/Models/TenantRequest.php#L19-L27). *(Cross-cutting — no single originating request.)*
...
- **REQ-4** 🟢 `[EXISTS]` — Full **audit trail** on status/assignment changes via spatie/activitylog. See [`getActivitylogOptions()`](../../app/Models/TenantRequest.php#L43-L50). *(Cross-cutting.)*
...
- **MNT-1** 🔵 `[NEW]` — Submission is restricted to the **tenant admin** (tenant side) and to operator staff (`channel = admin`). *Today any tenant-portal login can submit (`PortalRequestSubmittedNotification`); gating to a tenant-admin role requires tenant-users ([TEN-3](#7-tenants-company--users-ten)).* — *covers original request #9 (clarified: "admin" = tenant admin).*
- **MNT-2** ✅ `[DONE]` *(marker corrected 2026-08-12 — verified in code)* — `facility_work_orders.department_id` is a nullable FK to `departments`. A work-order is **assigned to a department** (and optionally a staff user or vendor). *Today assignment targets `User` (`assigned_to`) or `Vendor` (`assigned_to_vendor_id`); add a `department_id` dimension.* See [TenantRequest.php](../../app/Models/TenantRequest.php#L98-L106). — *covers #5.*
...
- **MNT-5** 🟢 `[DONE]` — A work-order that is **late/overdue** notifies **owner (Jawad) users** as oversight. **Implemented**: owner users (via `asset_owner`) are merged into the `requests:scan-sla-breaches` recipients (`AssetStaffRecipients::owners()`). *"Overdue" already = `isOverdue()` (open AND past `target_resolution_at`); the daily `requests:scan-sla-breaches` job fires `TenantRequestSlaBreachedNotification` once (idempotent via `sla_breach_notified_at`). Extend recipients to owner users.* See [isOverdue()](../../app/Models/TenantRequest.php#L118-L123) · [sla migration](../../database/migrations/2026_05_31_213931_add_sla_breach_notified_at_to_maintenance_requests.php). — *covers #4.*
...
- **NOT-1** 🟢 `[EXISTS]` — Channel-agnostic notification infra: Laravel notifications + `notifications` table + `DeviceToken` push. 9 notification classes exist (e.g. `TenantRequestSlaBreachedNotification`, `TenantRequestStatusChangedNotification`). See [app/Notifications](../../app/Notifications). *(Cross-cutting.)*
