...
- **[next]** remaining smaller gaps (`CheckIntegrationsCommand`, a few thin models) + the **specialised** testing the system has none of (Plan §E/§3 workstream E): security probes, performance/N+1 budgets, accessibility (axe), API contract/OpenAPI, RTL. These are larger, distinct workstreams — tackle one at a time.
