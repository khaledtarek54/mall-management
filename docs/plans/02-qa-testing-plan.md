...
- **[done] composer QA scripts** — `composer analyse` (PHPStan), `composer lint` / `composer lint-fix` (Pint over the WHOLE tree), `composer lint-dirty` / `composer lint-fix-dirty` (changed files only — the fast local loop), `composer qa` (analyse + full suite).
  - The whole-tree scripts became usable on **2026-08-17**, when the one-time reformat finally ran: 582 files were not Pint-clean, so any repo-wide run had until then produced a diff nobody could review and the scripts were scoped to `--dirty` as a workaround. `composer lint` now means what it says, and a drifted file shows up as a small diff instead of being lost among 582.
...
- **[done] Accessibility (axe-core)** — `tests/e2e/19-accessibility.spec.js` runs axe (WCAG 2.1 A/AA) over 6 surfaces (admin+portal login, admin dashboard, requests list, **create form**, portal dashboard), gating on CRITICAL. **It caught a real bug:** the language-switch pills (`<a>` links) used `aria-pressed` (button-only) → fixed to `aria-current`. Create form + list are fully clean. One `serious` color-contrast remains on Filament's "Powered by" footer (logged, non-blocking). *(commits, verified vs the Herd site)*
- **[next]** the lower-value remaining E2E items: RTL layout (the `dir=rtl` flip + AR labels are already covered by `06-locale.spec.js`; deeper component-mirroring is marginal), visual-regression snapshots (mostly guards future change), multi-browser (config). **The major automated test *types* are now covered** — the highest-value remaining work shifts to the **manual QA + UAT layer (Plan §F)**: a versioned test-case repo (`docs/qa/test-cases/`, every feature × role × path), exploratory charters, and UAT sign-off by the business. That layer needs a human driver.
