...
- **[next]** remaining **specialised** testing (Plan §E): performance / N+1 query budgets (assert a query-count ceiling on hot list/dashboard pages), accessibility (axe-core in Playwright), RTL layout checks. Each is a distinct workstream. (Per-endpoint response-schema validation via Spectator was assessed but deferred — the camelCase request/response duality makes it fiddly; the spec-completeness guard + existing per-endpoint feature-test assertions cover most of the value.)
