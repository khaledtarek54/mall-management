...
- **[next]** the remaining **specialised** testing the system still has none of (Plan §E): security probes (SQLi/XSS/CSRF/mass-assignment/rate-limit/webhook-HMAC), performance/N+1 query budgets, accessibility (axe in Playwright), RTL layout. Each is a distinct workstream — tackle one at a time. (Per-endpoint response-schema validation via Spectator was assessed but deferred — the camelCase request/response duality makes it fiddly; the completeness guard + existing per-endpoint feature-test assertions cover most of the value.)
