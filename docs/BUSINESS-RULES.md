...
| **Levy charge VAT-exempt** | Marketing charge is VAT-exempt, monthly frequency. Flagged for the accountant: a promotional-fund contribution is arguably a taxable service under VAT Law 67/2016 (your QA asked for 14% — Trello D0fZZ0fK, 2026-09-11). | A row on `/admin/charge-codes` (`marketing` → `VAT_14`) — the accountant rules it on the screen, no release; every lease bills it from the next run. | Documented (mirrors rent). | 🔴 HIGH | |
...
*Code references: `SlaSettings.php`; `config/sla.php`; `TenantRequestService.php`; `TenantRequest.php`.*
