...
> ### Status at 2026-08-12 — 31 closed, 14 open, and only 5 of those are engineering
>
...
> | What is left | Rows |
> |---|---|
> | **Engineering** | FS-21 (late-fee job re-entrant) · FS-22 (the concurrency gate) · FS-31 (SLA clock needs a click) · FS-32 (PPM can stop silently) · FS-33 (drift warns to /dev/null) |
...
> without. Of the five that remain, FS-33 (drift warns to /dev/null, `billing:reconcile` unscheduled)
> is the most valuable, because it is what would surface the next one of these on its own.
