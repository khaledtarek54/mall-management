...
> ### Status at 2026-08-12 — 37 closed, 8 open, and none of them is engineering
>
...
| ~~**FS-23**~~ ✅ | ~~**The authz gate is an `OR` where the rule is an `AND`**~~ — **shipped 2026-08-12**, at one seam instead of in 76 files. `Action::make()` resolves through the container, so `Filament\Actions\Action` is bound to `App\Support\Filament\AuthorizedAction`, which runs the authorization check inside `call()` — the check Filament's own `Action::call()` never performed. Every custom action's declared gate now actually runs at dispatch; `isAuthorized()` falls back to true when nothing was declared, so the ~500 actions gating with `abort_unless` are untouched. The test pins the refusal **and the binding**, so a Filament release switching `make()` to `new static` turns the build red rather than silently removing the layer. Three mutations, all killed | EDIT + gate | M |
