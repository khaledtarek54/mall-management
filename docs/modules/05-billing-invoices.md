...
**A catch-up run bills the month the lease was RUNNING, not the month it is being re-run in (SW-046, 2026-09-03).** `Lease::isBillableForPeriod()` and `scopeBillableForPeriod()` — the hand-synced pair that decides which leases bill — opened with `status === 'active'`, a fact about TODAY inside a question about a PERIOD. Every other clause in both is a date test. `leases:expire` runs at 05:15 and projects every active lease past its term to `expired`, so the morning after a term ended the lease dropped out of both halves for **every** month, including the months it had been trading through: `billing:run-monthly --period=2026-08` — the documented recovery from a failed billing night — put a lease that expired 31 August in the ordinary `skipped` counter, and `generateForLease()` answered `lease_not_billable`, so there was no manual way back either. The final month of every tenancy that ended between the failed run and the re-run was never invoiced. **`Lease::BILLABLE_STATUSES = ['active','expired']` is the one list, read by both halves.** `expired` is deliberately in it AND in `TERMINAL_STATUSES` because the two answer different questions — that one says the lease may no longer be CHANGED, this one says a month its term covered may still be INVOICED — and `Lease.php` already recorded the distinction in its own words: *"`terminated`, `cancelled` and `renewed` are each a person's act with a successor document; `expired` is a PROJECTION written by the nightly sweep."* Those three stay refused: a terminated lease has had its final account and its unearned credit note, and re-billing it would raise a second final invoice beside the credit note that closed it. **It widens WHICH LEASES are asked, never WHICH MONTHS bill** — the date clauses still require the period to fall inside the term, and `generateInvoiceForLease()` clips the trailing edge to `expiry_date`, so an expired lease can only be billed the part of its last month it actually ran. Note the scope's null-expiry branch is now GROUPED and narrowed to `active`: an ungrouped `whereNull('expiry_date')` ORs its way past the status clause, and `expired` with no expiry would bill for ever. Nothing writes to the lease during billing, so admitting a terminal status does not trip the immutability hook.


### SW-047

add beside the EG-30 arrears section:

**Ending an arrears charge still bills the month it consumed (SW-047, 2026-09-03).** An arrears row bills one cycle BEHIND the window it covers: August's service charge is settled on the September invoice, because until August has run nobody knows what the common area cost. `ChargeScheduleService::close()` writes `end_date = stop − 1 day` and, for a stop that has already arrived, `is_active = false` — and the planner bars an inactive row at the TOP of its selection, before the covered window is even computed. So ending a service charge "from 1 September" on the 2nd, on a mall whose billing day has not come round yet, forfeited the last month the tenant actually consumed: silently, and it is the operator's own revenue. **`end_date` was always the real bound** — the planner tests it against the window the row COVERS (August), not against the invoice's own period — which is exactly the reasoning `close()` already carried for a stop dated in the FUTURE (*"`end_date` alone is enough"*); it had simply never been applied to the half where the stop has arrived. The flag now waits `stop + billingCycleMonths` for an arrears row and drops on the spot for an advance one, so nothing an advance schedule does changes. **The residue is stated rather than left to be found**: a closed arrears row then sits `is_active` with a past `end_date` — exactly what the future-stop branch has always left behind. `state()` on the schedule tab reads it as *ended* from the dates and the planner refuses it, so nothing bills; what it does do is put such a row inside `pickInForce()`'s "latest active row" fallback, and inside `Charge::assertNoScheduleOverlap()` (which returns early for inactive rows), for one cycle. Both are pre-existing properties of the future-stop branch, not new ones. Compare SW-050, which is the same defect through `LeaseTerminationService`'s immediate-termination path and is NOT fixed here.


### SW-119

add as a new paragraph inside the `DocumentText` bullet (after the `TenantFacingWordingIsTheOperatorsConformanceTest` paragraph):

**One row per block per property — and the HOUSE row was the one nothing enforced (SW-119, fixed 2026-09-04).** The form scoped its `Rule::unique` with `$get('asset_id')`, but this screen's property control is `PropertyField::scope()`, a Radio whose blank state is the STRING `''` and not null — so the rule compiled to `asset_id = ''`, which can never match a NULL row. The index underneath is no help either: MySQL (like SQLite) permits unlimited duplicates where one side of a unique key is NULL. So a second house footer saved cleanly and `DocumentText::operatorText()` chose between the two by storage order — the "silent tie ... nobody's decision" the table's own migration says the index exists to prevent, and invisible from the panel because an operator editing one of them just sees the invoice keep the other one's wording. The form now clamps through `TenantScope::clampAssetId()` (null → `is null`, the same thing `HolidayForm` has always done), and `DocumentTemplate::saving()` is the guard an import, a seeder or a crafted payload meets. It fires only on a write that CREATES or MOVES the `(key, asset_id)` pair, because an install may already carry a duplicate and refusing every save would make both rows uneditable — putting the refusal's own escape out of reach.

### SW-201

under the tax section that introduces `tax_codes` + `tax_rates`:

> **A LIVE tax code keeps at least one rung (2026-09-04, SW-201).** `TaxCode::assertCanBeActivated()` refuses to switch a rate-less standard code ON, and until now nothing asked the same question in the other direction — the rate ladder is an ordinary relation manager with an ordinary Delete on every row. Measured on the seeded catalogue: remove the only rung of `STAMP_20` (active, standard, 20% since 2017-07-01) and `TaxCode::rateOn()` answers null, so `Vat::rateForType()` falls past its own floor into `standardRate()` and every supply the accountant had ruled carries 20% stamp duty originates at **14% VAT** from the next document on — no error, no toast, and the code still reading *active* on the screen it was emptied from. It is not an exotic state: on a fresh install all 7 `SCHD_*`, both `STAMP_*` and all 8 `WH_*` codes ship with exactly one rung.
>
> `TaxCode::needsARateLadder()` is the ONE predicate behind both halves, so activation and deletion cannot drift. The guard is on the MODEL (`TaxRate::deleting`), not on the button, so a second door onto the ladder is covered by existing rather than by being remembered — and it EXCLUDES the row being deleted, because during `deleting` that row is still in the table and a bare `doesntExist()` would refuse nothing while looking correct. A rung is still freely deletable while the ladder can spare it, on a switched-off code, and on an exempt or zero-rated one whose ladder `rateOn()` never reads; the refusal names both escapes. Known limit, stated rather than implied: a mass `$code->rates()->delete()` fires no model event — the same limit as the memo flush — and `app/` contains no such call. (`ALiveTaxCodeKeepsItsLastRateTest`.)


### SW-052

under the billing planner: "**A PLAN IS A READ: one broken schedule fails one lease, not four screens (SW-052, 2026-09-04).** The overlapping-schedule guard threw a `DomainException` straight out of `planInvoiceForLease()`, under a docblock claiming the damage was contained because `runForPeriod()` catches per lease. That was true of the two WRITE paths and of nothing else: every read caller loops the planner with no catch — `previewForPeriod()` (the Billing run preview, and through `MonthEndReadinessService` the Month-end close readiness figure) and `LeaseBillingForecastService` (the lease's Billing forecast tab, and through `PortfolioRevenueForecastService` the portfolio Revenue forecast). So one lease with two live rent rows took all four down for the whole property, including the tab an operator would open to diagnose that lease; and because a `DomainException` renders as a toast and a redirect rather than an error page, it read as \"the preview is broken\". `scheduleClash()` now ANSWERS the clash instead of throwing it, the plan carries it as the refusal reason `schedule_conflict` (worded in both languages in `billing_preview.reason` and in `BillingRefusal`), and `generateInvoiceForLease()` re-throws the same sentence so a broken schedule is still one `failed` lease in the run summary with the exception in the ops log — never a silent skip, and never a double charge. Fixed at the seam and not with a catch per screen, because a catch in the preview alone leaves the other three dead and cannot be falsified by a test."
...
**`invoices.status` IS A PROJECTION OF THE CALENDAR, AND NOW SOMETHING SWEEPS IT (SW-245, 2026-09-10).** `overdue` is derived by `Invoice::recomputeTotals()` from the four settlement channels and the due date — and that method runs when a SETTLEMENT lands, so an issued invoice nobody pays or penalises read `issued` indefinitely. Measured on the staging soak: six invoices two days past due, money on all six, all still `issued`; the four that did say `overdue` said it only because the late-fee run had touched them. **No money read the column** — SW-135 routed every collections surface through `stillOwed()` + the date precisely because the lag was known — which is why `ProjectedState` had carried it under `NOT_PROJECTED` with the question left open. What does read it is the register's status filter and its tabs, and the tenant's own portal view, and a screen that under-reports is the parking-bay finding again: no error, nobody files it. **The standard settles it**: `InvoiceForm` already treats `paid`/`partially_paid`/`overdue` as *"derived — not a choice, the next recompute corrects them"*, so a nightly recompute states nothing new; the SW-238/240 rule that a status past the first is the outcome of an ACT governs the act-statuses (`cancelled`, `credited`, `written_off`, `disputed`), which are the projector's own exclusion list and are never projected over. `billing:scan-overdue-invoices` — the sweep that exists to notice an invoice going past due, as `leases:expire` owns the unit projection — re-runs the projector on every row whose stored status disagrees with `pastDue()`, in BOTH directions: `issued` → `overdue` when the date has passed, and `overdue` → `issued` when a due date was EXTENDED as a concession and nothing recomputed. **The projector is `recomputeTotals()` itself**, so no second rule exists; the candidate set is written against the same predicate the projector reads (`Invoice::scopePastDue()` / `isPastDue()`, now the ONE spelling — `scopeOverdue()` composes it and `isOverdue()`, the mobile contract, reads it), so a second run finds nothing, which `ProjectedStateConformanceTest` requires of every sweep. **The boundary was moved two days later (SW-256, below)**: on 2026-09-10 the status still read `overdue` from midnight on the due day, as the display scope had always said, while the two chase sweeps followed Yardi's aging (due today is *current*) with `whereDate('<')`. Each row is re-read under a lock before the recompute, because `recomputeTotals()` also rewrites `paid_amount` and `balance` and every settlement path locks the invoice first. **Two fixtures fell over on it and both were unreachable states**: `paid_amount => 1000, balance => 0` written by hand with no receipt behind it, which the sweep honestly re-derived to unpaid — `settleInvoiceInFull()` now. (`AnInvoiceReadsOverdueOnTheDayItBecomesLateTest` — four mutations red; the under-lock status re-check is a concurrency control the suite cannot open and is recorded as such.)

**A DOCUMENT DUE TODAY IS CURRENT — IT IS LATE FROM TOMORROW, AND THE FEE'S GRACE ENDS THE DAY BEFORE THE FEE (SW-256, 2026-09-12).** Found on the staging soak at 06:00: three Nile Gate invoices due that day read **Overdue** on the register, in the portal and in the mobile app, while `AgingBuckets::keyFor()` — `days <= 0` is *Current* — filed the same three under *Current* on the ageing report beside them. `due_date` is a DATE cast, i.e. midnight, so `where('due_date', '<', now())` and `due_date->isPast()` were both true from 00:00 on the due day; the AR report, the AP badge and both chase sweeps already said `whereDate('<')`. Yardi's aging calls a document due today current. `Invoice::scopePastDue()` / `scopeNotPastDue()` / `isPastDue()` are the ONE boundary now, compared on DAYS (`whereDate('due_date', '<', today)`), and **four inline copies read the predicate instead of restating it** — the mobile app's `/me/balance` and `/me/summary` `overdue` figures, the tenant statement's and the owner statement's `summary.overdue` all carried their own `due_date->isPast()`. The SW-245 sweep's candidate set (`issued AND pastDue()` OR `overdue AND notPastDue()`) moves with it, so a stored `overdue` on a document due today is corrected at 06:00 and re-projected the next morning. **The review found the fee's door**: `LateFeeService::applyTo()` skipped only while `due + grace > today`, so with `late_fee_grace_days = 0` it charged ON the due day — a penalty on a document every other reader now calls current, whose status the 06:00 sweep would then have reverted, leaving a fee invoice standing against an `issued` parent. Grace days are days AFTER the due date with no penalty and the fee lands on the day after the last of them (`due + grace + 1`; the guard is `>=` now, and the batch selects through `pastDue()` rather than its own `<= today`). **That moves every late fee one day later on every install, deliberately** — 8th due, 7 days' grace, fee on the 16th, which is what the soak calendar had predicted — and no test had pinned the earlier day (`PerLeaseLateFeeTermsTest` names "Day 31" for due + 30 and checks days 10 and 31 only). (`AnInvoiceDueTodayIsCurrentNotOverdueTest` — eleven cases: the row twin, both query twins, the projection and its sweep, the two API endpoints, both statements, the ageing as the reader the status now agrees with, zero grace on the due day, grace 7 on the 15th vs the 16th, and the batch's selection pinned by its SQL shape because the grace guard alone would have kept the outcome green; nine mutations each killing its own tooth.)
