...
**A POOL'S IDENTITY AND ITS STAGE ARE FROZEN BY THE SAME THING THAT FREEZES ITS BASIS (2026-09-04, SW-161/SW-162).** The `updating` guard named five money columns and `CamExpensePoolForm` disabled eight fields, and between them they left the pool's own ADDRESS — `asset_id`, `period_year`, `pool_code` — and its `status` fully typeable. `period_year` is not a figure: `generateAllocations()` builds its 1 Jan → 31 Dec window from it and weights every tenure over that window, the billed estimate subtracts that year's invoices, and the recovery invoice the tenant is holding names it — so retyping it after billing re-keys a year of history under a heading the documents no longer match, and `applyEstimates` then proposes next year's monthly estimate from the wrong year. `CamExpensePool::IDENTITY_COLUMNS` is that set, read by `isDirty()` (which answers for ANY of an array) rather than re-listed, and refused with its own message so the operator is told which rule bit. **`status` is now the result of an ACT, never a field** — the same three lines as `TenantSalesDeclarationForm`'s status field (`->disabled()->dehydrated(false)`, which in Filament v4 really does drop the value from the payload, because `disabled()` calls `saved(false)`). A free Select there let anyone holding `cam.edit` write `reconciled` straight onto the row, skipping both halves of `markReconciled` — the `cam.mark_reconciled` permission and `assertReadyToReconcile()`, which refuses to close a year while a tenant's share is unbilled — and let it be typed back to `draft`, which re-opens `canGenerate()` on a pool that has already invoiced its shares, after which the scheduled `cam:reconcile` sweep regenerates it. **The bar is BILLING, not calculating**, honouring the 2026-08-31 decision in `generateAllocations()`: `CamExpensePool::hasBilledAllocations()` is now the ONE sentence saying so, read by the model guard and by `CamExpensePoolForm::basisFrozen()` (the collection-level `$isRerun` in the service stays as it is, deliberately, because it asks the same question of rows it already holds). **`closed` now has no writer at all** — it never had an act and the form was its only door; nothing behaves differently for it (`isReconciled()` treats it as `reconciled`, `applyEstimates` offers both, `voidAllocation` re-opens both), so the `closed` tab simply stops filling. A *Close the year* act is the follow-up; typing the column was not one.

### SW-170

**The statement's bottom line is the figure the tenant is ASKED for, VAT and all (SW-170, 2026-09-04).** A true-up leaves the building as one of two documents and both put the pool's `recovery_vat_rate` on top of the net figure — `billChargeImmediately()` raises the recovery invoice, `billCredit()` raises the credit note. `CamStatementPdfService::facts()` summed the true-up, the management fee and the fee's VAT and stopped, so \"Total now due\" understated a 50,000 shortfall by 7,000 on a pool at the shipped `default(14.00)` — on the one document RC-06 exists to let a tenant audit. The operator's Breakdown modal had it right the whole time, which is why the two were never held against each other. `CamAllocation::recoveryVatOn()` is now the ONE definition and all four readers take it — the invoice, the credit note, the modal and the statement — so a fifth cannot arrive at a fifth answer; the modal's credit branch also stops using `net × (1 + rate/100)`, which could disagree with `billCredit()`'s `net + round(net × rate/100, 2)` by a rounding step. **The credit side's arithmetic was in the TEMPLATE**, which is how it stayed wrong after the recovery side had been looked at: `facts()` is \"derived once so the view stays a layout\", so `net_credit` is a fact now. A pool at `recovery_vat_rate = 0` — the genuinely non-taxable pass-through — prints no VAT row and the same total it always printed, so nothing on an existing install moves.

### SW-167

> **⚠️ Σ SHARES ≤ 100% IS NOT THE SAME QUESTION AS Σ MONEY ≤ COST (2026-09-04, SW-167).** The F-08 guard refuses a reconciliation whose shares together promise away more than the pool, and it asks that in SHARE space while the allocation multiplies each share by the GROSSED-UP basis. `apportionmentBasis()` already refuses to gross up on an `occupied` denominator *"because there the shares already sum to 100%"* — and `occupied` was only a **PROXY** for that: a lease whose contract STATES its percentage (RC-03) fills a `gla` denominator back up without touching the basis. Measured: a 1,000 m² mall with 600 m² trading, a 1,000,000 pool at 60% variable grossed to a 95% assumption against 60% actual occupancy → basis 1,350,000; three 200 m² shops at 20% each with one stated at 60% → Σ shares exactly **1.000000**, the refusal passes, and Σ allocated is **1,350,000 against 1,000,000** of common cost — 350,000 billed to tenants for cost nobody incurred, with `landlord_unrecovered_amount` recorded as −350,000 and nothing refusing it (the tie-out is `actual − Σ allocated` by construction, so it holds throughout). **Bounded rather than refused**, because what is bounded is not somebody's agreed percentage — it is the landlord's OWN occupancy assumption, the same thing this method already declines to apply unilaterally — and the clamped figure is stored as `grossed_up_expense` and printed on the tenant's statement, so nothing about it is quiet. The bound uses the share this run will APPLY: the projected set on a first run, the FROZEN shares on a re-run, where a revised expense reaches the same arithmetic and F-08 deliberately does not run at all. **A zero share bounds nothing** rather than bounding everything to the bare total. `$basis` is now resolved AFTER the stated-share and carve-out resolution, because it depends on them. Pinned by `AGrossedUpPoolCannotRecoverMoreThanItCostTest`, mutation-proved three ways, including the control that gross-up still works.
