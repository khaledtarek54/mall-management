...
- **Its own `throttle:6,1,pay-demo`**, outside the `/pay` group's 30/min — two `throttle` middlewares on one route share a request signature and their counts interfere, so it is registered separately. **And it names its counter, because registering it separately was not enough** (corrected 2026-09-11): an unnamed throttle keys a guest on the IP alone, so until then "its own 6/min" was measured against the ONE count every unauthenticated route in the app was spending — the pay page's status poll, the mobile sign-in and the visitor feed included.
