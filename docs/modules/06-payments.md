...
- **It reuses `capturedCashPaid()`** — paid, net of credit notes and applied tenant credit — the
  same predicate `VoidInvoiceService` uses, named once so the two cannot drift. Since 2026-09-12
  the service and the *Void* button both read `Invoice::voidBlockedBecause()`, which composes it
  (see [module 05 → An invoice door reads the model's predicate](05-billing-invoices.md)).
...
- **Void auto-reverses applied credit.** Applied tenant credit is reversible non-cash, so it does **not** block a void — `Invoice::capturedCashPaid()` (`paid_amount − credit_applied_amount − Σ tenant_credit_applications`) is the single "real cash" predicate; `Invoice::voidBlockedBecause()` composes it with the terminal, ETA and write-off checks and is what `VoidInvoiceService` and the `void_invoice` button's `visible()` both read (2026-09-12 — until then the button carried a status ALLOWLIST beside the service's DENYLIST and hid the void on an invoice settled entirely by credit). On cancel, the Invoice `updated` hook soft-deletes the applications (returning the credit) just as it reverses applied credit notes — on **any** cancel path, so a credit can never strand on a voided invoice. Only captured *cash* still blocks a void (refund it first).
...
- **Its own `throttle:6,1,pay-demo`**, outside the `/pay` group's 30/min — it was registered separately because, unnamed, two `throttle` middlewares on one route shared a request signature and their counts interfered; with named counters they would not, and it stays outside so a demo press spends only its own budget. **And it names its counter, because registering it separately was not enough** (corrected 2026-09-11): an unnamed throttle keys a guest on the IP alone, so until then "its own 6/min" was measured against the ONE count every unauthenticated route in the app was spending — the pay page's status poll, the mobile sign-in and the visitor feed included.
