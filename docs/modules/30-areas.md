...
zones."* Zones already existed — `Area` has carried `asset_id`, a per-property unique code and a set
of supervisors since this module shipped — and were reachable ONLY from their own register under
Setup. Nothing new was invented: what was missing is the door, on the record the zones belong to.

`AssetAreasRelationManager` sits beside Floors on the property, because both are its spatial
make-up, and a zone's changes now appear on the property's Activity Log for the same reason.
`Asset::areas()` and `Area::units()` are the two relations that had never been written.

**It had already been claimed in a comment.** `AssetFloorsRelationManager`'s docblock stated that
*"units and zones are already managed this way"* — describing a screen nobody had built, which is
...
question CAM already settles. (`APropertyIsDividedIntoZonesFromItsOwnPageTest`.)
