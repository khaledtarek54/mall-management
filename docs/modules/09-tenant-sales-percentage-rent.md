...
- **The tenant is told it is one** (2026-09-11, mobile §L L4): `TenantSalesDeclarationResource`
  ...
  declaration the tenant filed — their "own" turnover, beside a percentage rent that read as reviewed
  (0 as raised, or computed from the estimate once an operator saved it). Not to be confused with the
  `is_estimate` in
