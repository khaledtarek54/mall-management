...
- **`recordPayment()`** — lock-safe, caps the amount at the balance, computes Egyptian **withholding tax**, and **guards `payment_date`'s period** the same way (the AP mirror of the AR receipt guard). A back-dated payment into a closed period is refused, not silently stranded. **The payment is a numbered document (2026-09-13)**: `VendorBillPayment` allocates `PMT-{mall}-{period}-NNNN` into `reference` on create — the series `DocumentNumbering::TYPES['vendor_payment']`, configurable beside the others, LENGTH-first like every series (EG-10), unique-indexed — and takes the **bank's** reference (cheque number / transfer reference) as `bank_reference`, the modal's optional field. Two facts, two columns, as the receipt keeps `RCT-…` apart from `cheque_number`. Until then `reference` was created for the number (its migration says *"e.g. BILLPAY-202607-0001"*) and written by nothing: every AP payment read *"Vendor payment #9"* on the ledger, *"—"* on the bill's payments tab and *"#9"* in the bank-reconciliation picker — found by driving AP as the accountant. The number is in the ledger narrative and the success toast; the bank reference is on the tab and beside each candidate in the reconciliation picker (`LinesRelationManager::bankReferenceOf()` — which also reads a receipt's `cheque_number`). Pre-existing rows were numbered in id order by the migration (`backfillMissingNumbers()`, idempotent); entries already posted keep *"#9"* in their narrative, because `LedgerPoster::matches()` compares lines, date and property and never text (EG-36). (`ASupplierPaymentHasANumberAndNamesItsBankReferenceTest`, six mutations — the receipt half of the picker read had no tooth until the review asked for one.)
