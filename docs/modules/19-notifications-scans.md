...
| `expenses:generate-recurring` | 05:30 daily | (no notification) | Books the costs due from `recurring_expenses` — real-estate tax instalments, municipal levies, a licence renewal. DAILY because the schedules do not share a day. Idempotent under a row lock, with `expenses.(recurring_expense_id, expense_date)` UNIQUE underneath, and it catches up ONE period per run so a reactivated schedule cannot post six back-dated entries at once (EG-33). | schedule due date vs `last_generated_on` | none |
