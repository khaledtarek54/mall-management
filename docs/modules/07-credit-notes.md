...
| **A STATUS IS THE OUTCOME OF AN ACT, and the form offered the acts as options (fixed 2026-09-01)**: every non-draft status here is derived or act-driven, and each act carries its own permission — `issued` from the Issue button (`credit_notes.issue`), `applied` derived from the remaining balance by `applyToInvoice()`, `void` from the Void button (`credit_notes.void`). **The status Select needed only `credit_notes.edit`**, so it produced all three effects with none of their checks, and a role deliberately denied `credit_notes.void` could void a note through it. Picking `void` skipped the applied-credit refusal, the balance zeroing, the `voided_at` stamp, the audit-trail reason and the GL reversal; picking `applied` marked a note fully spent while its balance still stood, making the credit invisible to every picker that narrows on `hasBalance()` — **the tenant simply loses it**. And the quiet one: picking `issued` skipped `PostingDate::assertOpen()`, so the AR effect committed while the journal post was silently refused inside the best-effort sync — *"Saved"*, and the ledger never moves. The picker is now narrowed on a DRAFT (where the choice means anything) and **disabled** once the note leaves draft, with `CreditNote::booted` as the gate — because a Select is a UI truth and the value still arrives in the Livewire payload. **The first cut removed the two options outright and was WORSE than the bug**, caught in review: Filament derives an `in:` rule from the options whenever it cannot resolve the CURRENT state's label, and an unresolvable state yields `Rule::in([])`, which nothing satisfies — so an applied or voided note became **unsaveable on every field** (an operator editing only `notes` got *"The selected status is invalid"*), and the single remaining option was `issued`, making the only way past that error to **UN-VOID the note**, which then prints with no VOID watermark on the document the tenant files. A **void note is terminal** now as well; nothing legitimately brings one back, and `voided_at` survives an un-void, so the blank-stamp discriminator would have been permanently satisfied for any note that took that route. Same shape and same reasoning as `cancelled`/`written_off` on the invoice form. | `CreditNoteForm` options + `CreditNote::booted` `updating`. | `AVoidIsAnActNotACreditNoteStatusTest`. |
...
| **A VOIDED note is not on the books, and the VAT return counted it (fixed 2026-09-01)**: `VatReturnService` filtered `whereNotIn('status', ['draft', 'cancelled'])` — and `credit_notes.status` has **no `cancelled`**. The set is `draft \| issued \| applied \| void`, so the filter excluded a status that cannot occur and counted every VOIDED note. **Excluding `void` outright is the obvious repair and is ALSO wrong**, in the direction that matters more — caught in adversarial review before it shipped. `JournalPostingService::void()` does not erase an entry: it posts a sign-flipped REVERSAL and marks the original `void`, and `void` is one of `JournalEntry::REPORTABLE_STATUSES`, so the original's VAT still counts in its own period. `reversalPeriod()` dates the reversal at the ORIGINAL entry date only while that period is OPEN, and at TODAY once it is closed. So a note issued in a month that has since been filed and closed, then voided, is netted in the CURRENT month — that month's ledger still carries the reduction, and dropping it from the documents side moves `base_standard` on an **already-filed return** by the whole credited supply. Measured: 60,000 → 80,000. The rule is therefore about DATES, in two halves that compose through the accumulator's sign: a note reduces this period unless its reversal landed inside it, and a reversal landing inside this period adds the reduction back (without which the month the void lands in is short by the reversal — the same permanently-red control, one period later). Reading the reversal's DATE does not make the tie-out tautological: every figure still comes from the document's own lines, and only *which period the correction belongs to* is taken from the ledger, because that depends on whether a period was open on a day that has since passed. `CreditNote::NOT_ON_THE_BOOKS` answers the narrower, timeless question of whether a note is a document at all, **derived by EXCLUSION** so an unknown status counts and must be excluded deliberately. | `CreditNote::NOT_ON_THE_BOOKS`, `VatReturnService::for()`. | `AVoidedCreditNoteIsNotOnTheVatReturnTest`. |
