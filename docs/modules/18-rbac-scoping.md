...
- **Privilege-escalation guard**: only a super_admin may grant or revoke the `super_admin` role — `UserResource::guardSuperAdminAssignment()` forces a non-super_admin's submitted role set to preserve the target's current super_admin status (the UI also disables the option). 
