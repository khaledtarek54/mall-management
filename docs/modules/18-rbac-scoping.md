...
`, as a new `### Filtering the trail by WHO acted (2026-09-04)` subsection at the end of §7, immediately BEFORE the line `## 8. Extension points — how to change/extend SAFELY`:

### Filtering the trail by WHO acted (2026-09-04)

`App\Support\Filament\CauserFilter` is the ONE definition of the audit trail's most-asked filter, read by both screens that render the feed: the per-record **Activities** tab (`ActivitiesRelationManager`) and the portfolio-wide **Activity log** page. The tab has been filterable by causer since it shipped; the page — the one an auditor is actually sent to, and the only one that delivers a scheduled CSV — offered `log_name`, `event` and two date controls and nothing for the person. Measured on the dev database 2026-09-04: 473 rows, 32 of them carrying a causer, and no control on that screen could narrow to them. Mounting the filter answers the CSV in the same move, because `ActivityLog::reportCsv()` exports `getFilteredTableQuery()` — one seam rather than a screen fix and an export fix free to disagree.

**The `causer_type` clause is not optional, and that is why the definition is shared.** `causer` is a `morphTo`, and a causer is not always a `User`: a contractor accepting a job through the vendor portal is a `VendorContact` (`AcceptWorkOrderService::accept()` takes a bare `Model` and is called with `VendorScope::contact()`). `users` and `vendor_contacts` are independent id sequences — measured, `users` holds ids 1-6 and `tenant_users` holds id 1 — so the obvious `where('causer_id', $id)` returns another table's rows under the name of the person the operator picked. On an audit trail that is the worst available failure: it does not look empty, it looks answered. Both halves live in one class so neither screen can hold half the rule.

**There is deliberately no search box.** Every column on both tables is derived at READ time — the event word, the subject's label, the rendered change set — so no stored text exists for a `LIKE` to reach, which is why the tab sets `->searchable(false)` explicitly. Typing into the causer picker IS the search, against the folded `search_text` blob, which is why the control is an `EntitySelectFilter`. `TextColumn::make('causer.name')->searchable()` is not the alternative: `MorphTo` declares no `getRelationExistenceQuery()` and inherits `BelongsTo`'s, so Filament would build the constraint against `activity_log` itself.

**Known bound, stated rather than implied:** the options are `User` rows only. A `VendorContact` causer still appears in the Who column and is still exported, but cannot be picked — `EntitySelectFilter` binds exactly one model, and a composite `type:id` option key would forfeit the folded-blob search. The way to that contact's actions is the unfiltered feed or the work order's own Activities tab; widening it is a change to `EntitySelectFilter`, not to a call site. (`TheAuditTrailCanBeFilteredByWhoActedTest`.)


### SW-098

**THE RBAC VOCABULARY IS A CATALOGUE, AND ITS ENGLISH HALF IS DERIVED (2026-09-04).** `RolesPermissionsSeeder::PERMISSIONS` is `key => English sentence` and `RoleForm` handed it straight to `CheckboxList::options()`, so **232 checkbox labels** on the screen that decides who may do what rendered in English on the Arabic panel — measured at 83624504, with `trans('admin.permissions', [], 'ar')` returning the literal key because no such catalogue existed in either locale. The SECTION headings were already right (`admin.permission_modules`, 59 entries in both), which is exactly why this survived: `Translate`'s docblock records fixing *"~110 strings on one screen"* and that round stopped at the headings. Two more surfaces had the same shape — the role-description column on the Roles list (14 sentences off `ROLES`) and both role PICKERS on the Users screen, which label an option with the relationship TITLE ATTRIBUTE, i.e. the raw identifier `hr`, while the badge column beside them renders «الموارد البشرية». `App\Support\PermissionVocabulary` is the one seam all six call sites read. **The English half is BUILT from the registry** (`lang/en/admin/permissions.php` loops `PERMISSIONS`), so there is no second English wording to drift, and `TranslationKeyConformanceTest` test B then makes a permission with no Arabic a build failure on arrival. Three things to know before touching it: the catalogue keys must be **nested** (`__()` reads a dot as a path — test G), `Lang::has()` **falls back to English** so any parity check here must pass `fallback: false`, and a filter's CHIP is a **third** rendering that reads neither the options nor the option-label callback — `SelectFilter::setUp()` plucks the title attribute off the column, so it needs its own `indicateUsing()`. While measuring, one inert line was removed: the users role filter's `->options(fn () => Role::pluck('name','name'))` was never read (a filter that `queriesRelationships()` serves its dropdown from the relationship), and just as well, because those keys were names while `apply()` ends in `whereKey()`.
