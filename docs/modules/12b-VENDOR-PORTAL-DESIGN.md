...
### The profile page is the panel's own (2026-09-04)

`VendorPanelProvider` registers `App\Filament\Vendor\Auth\EditProfile`, not Filament's. Filament's stock page builds the email field as `->unique(ignoreRecord: true)` against the panel's authenticatable model, which for this panel compiles to `unique:vendor_contacts,email,"<id>",id` — the WHOLE table. That is correct on the other two panels and wrong here, and the difference is in the schema rather than in anybody's opinion: `users.email` and `tenant_users.email` each carry a real UNIQUE index, `vendor_contacts.email` deliberately carries none, because two contractors may share a switchboard address and uniqueness is required only among rows that can SIGN IN. **The consequence was not a refused email change — it was a refused SAVE.** The page writes name, email and password in one submit, so a contractor whose address also sat on an ordinary non-login contact could never change their own password, refused on a field they never touched. `VendorContact::constrainToLogins()` is now the ONE expression of "the rows a login email contends with", read by the model's `saving` guard as an Eloquent scope and by the form's `unique` rule as a raw query-builder clause; it is typed to `Illuminate\Contracts\Database\Query\Builder`, which both satisfy. **The override rebuilds the component rather than chaining onto `parent::`**: `CanBeValidated::unique()` APPENDS a rule and nothing can remove one, so chaining would leave the unscoped rule standing while reading as a fix — mutation-proved.
