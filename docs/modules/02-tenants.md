...
- **Gotcha:** Once migrated, the `Tenant.password` field is still there but should not be used **for the portal** — portal auth goes through `TenantUser`. Do not encourage tenants to update the password on the Tenant record; they should only update it in their TenantUser account.
- **⚠️ Fixed (2026-08-11): the tenant import deduplicated on a nullable column.** `TenantImporter::resolveRecord()` keyed on `tenants.email`, which is **nullable with no unique index** — so every email-less tenant was unrecognisable on a second pass and got a fresh row. Re-running an import is the normal response to a partial one, and the duplicates are not harmless: each acquires its own leases and invoices, splitting one retailer's AR across two records that can no longer be merged, because `RefusesDeletionWhenReferenced` correctly refuses to delete either once it has history. Egyptian retailers routinely appear on a roster with a tax registration and no email. Identity is now **`tax_id` first, `email` as fallback**, normalised through `Tenant::normaliseTaxId()` — the same rule the mutator applies on write, because the column stores bare digits and searching for the dashed `123-456-789` the CSV carries would have matched nothing, **creating a duplicate of every tenant while appearing to prevent exactly that**. Pinned by `TenantImportIdentityTest`.
- **⚠️ Correction (2026-08-11): `Tenant.password` is NOT vestigial.** It is the live credential for the **mobile API** — `/api/v1/auth/login` authenticates the *company* through `LoginTenantAction` against this exact column. Reading the line above as "this column is dead" is how a real defect survived: `LeaseCreationService::createTenant()` did `Hash::make($data['password'] ?? 'password')`, and the quick-lease wizard has no password field, so **every tenant onboarded through the wizard shared the literal secret `password`** — knowing a retailer's email address was enough to authenticate as that company. It now generates an unusable `Str::password(32)` when none is supplied: the company exists, nobody can sign in, and the operator issues real credentials through the super_admin/manager-gated *Set Up Mobile App Access* action — **renamed from *Setup Portal Access* on 2026-09-04**, because the old label and its modal told the operator in both languages that the tenant "will log in at /portal with their email", which this credential can never do. It was reported from staging as a portal login that "always gives wrong creds", and Filament answers a wrong password and an unknown account with the same sentence, so it cannot be told apart from the outside (`ATenantsAppPasswordIsNotAPortalLoginTest`). Pinned by `QuickLeaseTenantCredentialTest`. Note the two surfaces still differ in kind — the mobile API authenticates a **company** with one shared secret, while the portal authenticates **people** with an `is_admin` write gate; closing that asymmetry is a separate, larger piece of work.
