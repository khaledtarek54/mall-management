...
|---|---|---|
| **Status gate:** Only `status='active'` tenants can access the Portal (Filament `canAccessPanel()` gate). `inactive` and `blacklisted` tenants are immediately blocked. | `Tenant::canAccessPanel()` checks `panel->getId() === 'portal' && $this->status === 'active'` | `TenantUserGatingTest` — admin+read-only permissions tied to TenantUser.is_admin, TenantUser must belong to active Tenant |
...
| **Identity fields (tax_id, national_id, commercial_register) are collected for ETA/legal filing.** At ETA submission, tenants must have valid tax_id. | Form validation: `tax_id` matches regex `/^\d{3}-?\d{3}-?\d{3}$/` (Egyptian VAT format; audit M15 F-59). Commercial register, national ID, legal name are collected but no per-tenant validation rule enforced (validation happens at lease/invoice submission). | TenantForm schema validates tax_id format. No single test for identity fields (they flow to ETA layer). |
