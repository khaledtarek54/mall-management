...
append to the EG-03 blockquote that ends *"Pinned by `PayrollRatesAreDatedAndCappedTest`"*:

> **And until 2026-09-04 the band could not be ENTERED (SW-203).** The ceiling carried `->rules(['gte:insurable_wage_floor'])`, and a rule string reaches the validator verbatim — measured, the field compiles to `['nullable','numeric','min:0','gte:insurable_wage_floor']` — while the attribute it is keyed under is `data.insurable_wage_ceiling`, because a resource form lives at the `data` state path. So `Validator::getValue('insurable_wage_floor')` read the ROOT of the Livewire payload, found nothing, and `validateGte()` fell through to `isSameType('16700', null)`, which is false. Measured on that exact rules array: **every** filled ceiling was refused, including the 2,700 / 16,700 rung the migration itself inserts — so the January decree, on a ladder whose whole point is entering a rise in advance, was the case that could not be saved. The row that reported it named only the narrower half: a ceiling with a blank floor. That half is real too and survives fixing the path, because `validateGte()` also answers false against a null comparison value, while **a null bound is NO bound** — `insurableWage()` skips a null floor entirely and a ceiling-only rung is legal. Both halves are one closure rule now, stated where they are visible together; `->gte()` would have fixed only the first. The guard stays on the FORM rather than the model: the form is the only door onto this table, and a field-level refusal beats a toast that loses what the operator typed — if an importer ever appears, the rule moves to `PayrollRate::saving`. (`AnInsurableWageBandCanActuallyBeEnteredTest`.)
