...
**A unit sale opens on the mall's payment-terms convention (SW-056, 2026-09-04).** `UnitOwnershipForm` pre-fills `payment_terms_days` from `PropertySettings::paymentTermsDays(TenantScope::currentAssetId())` — property → portfolio — exactly as `LeaseForm` has since EG-35, and no longer from a literal 7. The column is NOT NULL with a database default, so a `?? setting` fallback in `UnitOwnership::paymentTermsDays()` could never fire (`SettingsReachConformanceTest` records that dead-fallback trap by name); the convention therefore belongs at ORIGINATION and nowhere else, which that method's own docblock already said — describing a form pre-fill that did not exist. From then on the sale carries its own number, so revising a mall's convention cannot move the due date on assessments already raised. **The model's `$attributes` default of 7 deliberately stays**: it is a static array with no property in hand, it mirrors the NOT NULL column so an unsaved `new UnitOwnership` answers what the database would, and a second answer there would conflict with the form's. **`TransferUnitOwnershipService` also stays as it is** — a resale copies the SELLER's terms, because it inherits the arrangement that was agreed rather than today's default. Doors enumerated by grepping the column: the form is the only one that mints a fresh ownership (there is no unit-ownership importer). The field also gained the `admin.fields.days` suffix its lease twin already carries, so a mall's 30 does not read as an unlabelled number.


### SW-229

**A UNIT MAY NOT BE OWNED MORE THAN ONCE OVER (SW-229, 2026-09-04).** `UnitOwnership::saving` checked a row's own two dates and never its siblings', and no unique index could: a unit legitimately carries several ownership rows. `TransferUnitOwnershipService` writes `ended_at = $on->subDay()` so the service never produces an overlap; the **register's own form** offers free date pickers and is the door (there is no unit-ownership importer). Measured on a 100 m² unit in 2026: a seller ending 1 July beside a buyer starting 1 July gives the unit **366 owner-days of a 365-day year** — `areaSqmForPeriod()` weights m²·days, so the unit contributes 100.274 m² where it measures 100, taking **50.068%** of a two-unit CAM pool against an identical full-year unit's **49.932%**, and `BillUnitOwnershipsService` bills the shared day to both owners. Outbound over-recovery on the monthly assessment and on the annual true-up, and structurally invisible: Σ allocated still equals the actual expense, so the pool ties out throughout. **The rule is NOT "no two tenures may overlap"** — overlapping is the ordinary state of a CO-OWNED unit (`ownership_share_pct`, and `DemoSeeder` seeds 60/40), and a `contracted` sale running alongside the current owner's live tenure is the ordinary shape of a pending resale. `UnitOwnership::assertUnitIsNotOwnedTwiceOver()` refuses only when **the shares in force on one day exceed 100%**, over the states DERIVED from `UnitOwnershipStatus::everHadPossession()` rather than re-listed; `reserved` and `contracted` bill nothing and reach no pool, so they are outside it. The sweep evaluates the candidate days clipped into this row's own window — the maximum of a set of intervals is attained at one of their starts — with null bounds clamped to sentinels so it has no branches, and a 0.005 tolerance so three co-owners at 33.34/33.33/33.33 are not refused for a hundredth of a percent. **Refused only on a DIRTY write** (`unit_id`, `started_at`, `ended_at`, `ownership_share_pct`, `status`), for the reason `RecordsBankAccount` states: a pre-existing overlap must not make either row uneditable, and the corrections that clear it are themselves the write the guard evaluates, so the escape the message names is real. **A PLAIN read, deliberately** — the hook often runs with no transaction around it and a row lock taken there is released on the next statement, which is worse than none because it reads as protection; the concurrent double-transfer is already held by `TransferUnitOwnershipService`'s `lockForUpdate()` on the seller. Same reasoning as the deposit cap on `DepositTransaction::saving`. (`AUnitIsNotOwnedTwiceOverTest`, mutation-proved four ways.)


### SW-015

append to the "lease is the route to the property" section (the table around line 459):

**And the lease was the route to the UNIT, on five more surfaces (2026-09-04).** `Invoice::unitCode()` was written on 2026-08-18 for the two invoice TABLES, because a table column can take a `->state()` closure; its commit message said "two surfaces ask it". Five others name a COLUMN instead of running a closure — the portal invoice **View page**, the top-bar **search result**, the **AR-ageing worklist**, that worklist's **CSV**, and the invoice **export** — so all five went on reading `lease.unit.code` and printed a blank for every owner assessment, including on the one screen the owner himself opens to read his own bill. Measured on `mall_management_qa` 2026-09-04: 42 of 290 invoices carry a NULL `lease_id`, so one invoice in seven showed a dash where its shop belongs. Nothing failed: a blank cell reads as "no data", not as a bug. `Invoice::getUnitCodeAttribute()` is the fix — `unitCode()` under a name every renderer can say — a second ROAD to the rule and never a second answer, and deliberately **not** in `$appends`, because `docs/api/openapi.json` is generated from `toArray()` and the API already publishes `unit_code` explicitly. `ReportService::openInvoicesAsOf()` and `InvoiceExporter::modifyQuery()` eager-load both agreements, or the fix buys a correct cell at the price of a query per owner row on the one dataset that never shrinks. (`AnOwnerAssessmentNamesItsUnitEverywhereTest`, mutation-proved six ways.)
