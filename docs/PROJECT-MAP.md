...
| | | | |
|---|---|---|---|
| **132** models | **183** services | **68** admin resources | **9** portal resources |
| **61** API controllers | **19** dashboard widgets | **66** console commands | **6** queued jobs |
| **39** notifications | **327** migrations | **39** module docs | **1377** test files |

### Coverage — what the gates actually enforce

| Dimension | State |
|---|---|
| **Property isolation** | 132 models classified — 93 property-owned, 38 shared, 1 self. Gated by `PropertyIsolationConformanceTest`. |
| **General ledger** | 25 posting sources, 25 journalizer classes — every one registered. One registry (`LedgerPoster::JOURNALIZERS`) that all four dispatch paths derive from; gated by `GlRegistryConformanceTest`. |
...
| **Tests** | 1377 files — 230 scenario, 876 regression. |
| **Module flags** | 36 toggleable modules (`App\Support\Modules`). 1 frozen in code and shown nowhere: `eta`. |
...
**39** modules carry a doc in [`docs/modules/`](modules/). Every one of them has been gap-analysed at least once; the standing verdict per module lives in [`docs/gap-analysis/`](gap-analysis/README.md), which is one living document rather than a file per module.
