...
<div class="rule"><span class="lbl">The bridge · LedgerPoster + ~20 journalizers</span>A nightly sweep (<code>accounting:sync-ledger</code>) runs each document through its journalizer and <b>self-heals</b>: if a document changes (a late fee bumps an invoice total), it voids the stale entry and re-posts the new one; if a document loses its effect (cancelled, refunded, deleted), it voids the entry. You've seen ~20 of these translators across the handbook — this is where they all land.</div>
