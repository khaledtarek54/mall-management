...
| **F-96** | Facility maintenance | `facility:scan-sla-breaches --dry-run` assessed penalties (wrote real `sla_penalties` rows) *before* the dry-run check. It now previews and returns before any write. | `WorkOrderSlaDryRunTest` |
