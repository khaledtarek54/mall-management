...
| B3 | **An owner-occupier cannot hold a parking bay** — DECIDED 2026-08-19, not yet built | 35 · 37 | **Decision made, so this is no longer blocked — it is scheduled work.** The operator ruled that an owner CAN hold a bay and that the charge **rides the monthly صيانة assessment run**, like the rest of their charges. **Shape:** `lease_rentable_item` becomes polymorphic over `BillableAgreement` (holder_type/holder_id, backfilled from `lease_id`), the assign service takes an agreement rather than a `Lease`, and the relation manager appears on both holders. The money half is already ready — `charges.unit_ownership_id` exists, so an ownership can carry a `parking` charge today. **Not the lease-shaped-assumption class** module 37 wrote up: those inferred a lease where the money core already took a `BillableAgreement`, so the fix was a lookup. Here the RELATIONSHIP is lease-shaped, which is a migration and a backfill. **Sized honestly: 19 files under `app/` and `tests/` read the lease pivot**, and it is a live billing path — worth doing as its own change with the parking-billing regressions green, not as the tail of another one | M |
