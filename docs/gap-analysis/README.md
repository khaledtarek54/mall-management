...
| # | Item | Why | Effort |
|---|---|---|---|
| ~~H1~~ | ~~**Switch on `FixtureColumnsExistConformanceTest`**~~ **DONE 2026-08-24 (`7335552f`)** — the 72 ghost fixture keys were cleared in one pass and the `skip()` removed the same afternoon, hours after this row was last re-verified. It was right about the method: turning a build red on an inherited backlog is how a gate gets deleted rather than obeyed, so the ghosts went first and the gate second. [STATUS §0](../STATUS.md) records the same correction | ✅ |
...
| Escalation: per CHARGE, not only the rent | ✅ Yardi's escalation schedule is per charge code | ✅ **Built 2026-09-12** (meeting 2026-09-02, point 24). Every recurring charge row carries its own rule — follows the rent's clause · its own % · its own fixed EGP · none — swept on the lease anniversary and projected at signing; the lease form's *Annual increase* tab rules on each. **Stated deviation:** one anniversary and interval per lease (Voyager allows a frequency per charge; no Egyptian clause needs it). `billing.new_charges_follow_escalation` per property, off = Yardi | ✅ KEEP | ⚪ |
...
| **Aged payables** | ✅ | ✅ **BUILT 2026-09-12 (the reports audit).** `/admin/ap-aging` — per supplier, aged by due date on the receivables' own buckets, worst-first, with the register's unpaid bills one click away and the AP control-account tie-out on the subheading through `glTieOut()` itself (scoped to the mall on screen). It was the one standard financial report the catalogue had no counterpart to: the bills carried `due_date` and `balance` and the reconciler already tied AP out, and nothing read either for a person. See [modules/17 → Aged payables](../modules/17-reports.md#aged-payables-2026-09-12-the-reports-audit) | ✅ KEEP | ⚪ |
...
| **A parking bay steps by its own rule, is let with the lease, and is ruled on from the tab: BUILT (2026-09-12)** | The register's half of Yardi's per-charge escalation (point 24 had made the parking row derived and left the bay's step for the register): the holding carries the item's own annual increase, `RentableItemPricing::rateOn()` is the one arithmetic the projection, the sweep and the assignment-day re-sum read, the parking rows are re-laid from the register rather than amended, a lease is created with its items through the one assignment door (form and quick wizard), and the rule is set from the schedule and items tabs with the form's table refilling. Two pre-existing register defects closed on the way — a future-dated release billed nothing from the day it was recorded, a re-let bay was closed and summed twice ([modules/35](../modules/35-rentable-items.md)) |
