...
| B2 | **Which properties are publicly listed?** | 36 | The operator: default-listed (nothing changes) or default-unlisted (the shopper feed empties until each mall opts in). There is no `is_publicly_listed` column, so withholding a mall today means deactivating it, which breaks every operational screen | S |
| B3 | **Can an owner-occupier hold a parking bay?** | 35 · 37 | The operator: whether a bay charge rides the owner's monthly assessment. `lease_rentable_item` is lease-keyed, so today he cannot hold one | M |
