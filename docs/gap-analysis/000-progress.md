...
> **Closing pass (2026-07-18): the generic modules are now CLOSED.** The batched 🟡 correctness/
> consistency tail — F-90b, F-91, F-96, D-95, D-96 — is fixed, so the generic-ERP layer has **zero
> open correctness findings**. It is deliberately frozen; further build effort goes to property +
> facility. See **[GENERIC-MODULES-CLOSED.md](GENERIC-MODULES-CLOSED.md)** for the line: what
> "closed" means, what stays parked (and its trigger), and the one accountant question
> (employer social-insurance / gratuity) that could still move the books.

| # | Module | Status | Doc | Headline |
|---|---|---|---|---|
| 21 | General Ledger | 🟡 Yellow | [21-general-ledger.md](21-general-ledger.md) | **F-79 FIXED** — `matches()` ignored `entry_date`, so a date-only edit stranded the entry in the wrong period, undetectably (the close gate and `--deep` reconcile were blind **by construction**, since `wouldChange()` reuses `matches()`). The sibling of the SlaPenalty bug that survived the registry fix: not a missing *source*, but a source **field the reconciler never compared**. |
...
| 23 | Fixed Assets | 🟡 Yellow | [23-fixed-assets.md](23-fixed-assets.md) | **F-86 FIXED** editing `acquisition_cost` below posted accumulated gave **negative NBV, depreciation stops forever** — the clamp only protected the forward run, never a retroactive re-cost. Now `assertRecostValid()` refuses it. F-87/F-88 (period/classification) remain. |
| 24 | HR / Payroll | 🟡 → 🟢 | [24-hr-employees.md](24-hr-employees.md) | **F-90b FIXED** a payroll *line*'s net could go negative (the guard checked only the header) → a payslip printing **Net −1,000** on a frozen run; now guarded per line. **F-91 FIXED** a mis-keyed repayment was permanently uncorrectable; now `RecordAdvanceRepaymentService::reverse()` + a Reverse action (mirrors custody F-94). Both closed 2026-07-18. |
| 25 | Treasury / Custody | 🔴 → 🟢 | [25-treasury-custody.md](25-treasury-custody.md) | **F-93 FIXED** — a settlement dated into a closed period **silently diverged the GL from outstanding**, and back-dating across a close is a عهدة's *normal* workflow, not an edge case (the queued job swallows the throw; the operator is told it succeeded). Now refused by `App\Support\PostingDate`, which closes F-89 in module 24 too — one bug, two hats. **F-94 FIXED** a Reverse action (soft-delete = void) — the correction path every other money document already had. |
| 26 | Facility Maintenance | 🔴 → 🟢 | [26-preventive-maintenance.md](26-preventive-maintenance.md) | **F-77 FIXED** a penalty could be charged to **another property's** vendor bill (wrong mall absorbs it; cross-property read leak in the picker). **F-78 FIXED** waiving an *applied* penalty never recomputed the bill → bills say 9,000, GL says 10,000, **AP tie-out breaks and the vendor is underpaid**. **F-96 FIXED** (2026-07-18) `--dry-run` wrote real penalty rows; it now previews before any write. |
...
| 09 | Maintenance / CAFM | 🟡 Yellow | [09-maintenance.md](09-maintenance.md) | F-17 carryover fix on both badge methods. 3 Yellow: F-36 SlaSettings SLA props are unused (service reads config), F-37 no notifications on status/SLA, F-38 auto_close_after_days never acted on. 17 Pest + 3 e2e green. |
...
- **3 Yellow findings (deferred)**:
  - **F-36**: SlaSettings has 4 SLA props that are NEVER read by anything — service reads `config('maintenance.sla.*')` instead. Numbers diverge (Settings 4h urgent vs config 24h urgent). Either delete props or wire service to Settings (recommend latter for operator tunability).
