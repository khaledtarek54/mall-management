...
| ⚠️ **DEFER — APPLYING a tenant credit to an invoice** — a manual apply was built then **reverted before shipping**: the pre-push adversarial review found (CRITICAL) that extending the source payment's pivot re-derives its **immutable, possibly closed-period** GL entry (`entry_date = payment_date`); a prior-period overpayment applied to a current invoice would drop AR while the GL post is refused (closed period) → permanent GL↔AR divergence with a false success toast — the exact "cut the balance while posting nothing" class the GL invariant forbids. **Correct design:** a NEW correcting entry **Dr Unearned Revenue / Cr AR dated at application time** (open period) as its **own** `LedgerPoster::JOURNALIZERS` source (+ `SOURCE_DATE_COLUMNS` + a GlRegistry-gated tie-out test driving the real sweep), settling the invoice via a dedicated summand (NOT `credit_applied_amount`, which is credit-notes' Dr Sales-Returns/Cr AR — would mislabel the GL). | **business_logic/high** | 06 | operators need to draw down a credit. Do it as the explicit dated-now GL source above — never by re-deriving the historical payment. |
...
| **DEFER — auto-apply a credit on next invoice issuance** — hook the billing run, idempotent/lock-safe, on top of the correct apply GL source above | **business_logic** | 06 | prepayment volume makes hands-off desirable. |
