...
| **Fit-out / rent-free grace period on the lease** — near-universal Egyptian pattern; today faked by hand-setting each charge's `start_date` (billing engine already honours it) | **ux+business_logic · DOMAIN CALL** | 04 | ⚠ **gated on `OPEN-QUESTIONS.md C1.5`:** which charges suppress during fit-out (rent only? +service/CAM/levy?) and field shape (date vs `fit_out_months`). Then a small additive build. |
