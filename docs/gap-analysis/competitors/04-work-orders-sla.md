...
> [`docs/modules/26`](../../modules/26-preventive-maintenance.md) (`MaintenanceWorkOrderService`,
> `MaintenancePlan`/`MaintenanceWorkOrder`, `Equipment`, `SlaResolver`, `sla_policies`,
> `MaintenancePenalty`, `WorkOrderPartService`) and [`docs/modules/11`](../../modules/11-maintenance.md)
...
|---|---|---|---|---|---|---|
| PPM scheduling (calendar/frequency) | ✅ | ✅ | ✅ | ✅ | ✅ | `maintenance_plans` frequency `days\|weeks\|months\|years × N`; daily `maintenance:generate-preventive` raises WOs + advances `next_due` (idempotent, lock-safe). Parity on the routine case. |
...
| Checklist / job-plan template on a WO | 🟡 | ✅ | ✅ *(verify)* | ✅ (job plans + labor/tools) | ✅ (procedures) | `plan.checklist` (json) copies into `maintenance_work_order_items`. A pass/fail item list — **not** a job plan with labor estimates, crafts, tools, safety steps. |
...
| WO lifecycle / enforced state machine | ✅ | ✅ | ✅ | ✅ (deep) | 🟡 *(verify — lighter statuses)* | `MaintenanceWorkOrderService::TRANSITIONS` (`open→in_progress→done\|cancelled`); illegal hops throw, terminal orders frozen (checklist + edit). |
...
| SLA breach detection + alert | ✅ | ✅ | ✅ (compliance tracking) | ✅ (escalations) | 🟡 (overdue alerts) | Hourly `maintenance:scan-wo-sla-breaches`, idempotent (`sla_breach_notified_at`), row-locked, bell alert. Stamps even when nobody's assigned. |
| **Vendor SLA penalty auto-computed + charged to AP/bill** | ✅ | 🟡 *(verify)* | 🟡 *(verify — invoice deductions)* | 🟡 *(verify — contract terms)* | ❌ | **Atriom's standout.** `MaintenancePenalty`: 3 bases (flat / per-day accrual / %-of-job-value), one re-assessed row per WO, frozen on closure, waivable; **charged onto the vendor bill as an AP offset** (`penalty_applied_amount` in `VendorBill::recompute()`) and GL-posted Dr AP / Cr expense. External jobs only. |
...
| Parts/inventory draw on WO + approval | ✅ | ✅ | 🟡 *(verify)* | ✅ (storeroom/reservations) | ✅ | `maintenance_work_order_parts`: internal draw **requested→approved** (approver by value via `ApprovalPolicy`, frozen on the row, self-approval refused), external recorded; stock movement + property-dimensioned GL. |
