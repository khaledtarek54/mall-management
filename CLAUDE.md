...
- **CI auto-runs are ON** (re-enabled 2026-07-29; they were paused 2026-07-26 → 2026-07-29). `.github/workflows/ci.yml` fires on `push` + `pull_request` to `main`/`develop`, plus `workflow_dispatch`. Jobs: `composer audit` · PHPUnit · PHPStan · Playwright. The five self-enforcing conformance gates (PropertyIsolation, GlRegistry, MediaPrivacy, AdminSmokeManifest, **DashboardLayout**) run inside the test job and are **enforcing again** — a bad change goes red instead of going quiet. *(Why they came back: while paused, a widget carrying no authorization gate at all shipped and published the property's receivables to the HR and marketing dashboards — precisely the class those gates exist to catch.)*
